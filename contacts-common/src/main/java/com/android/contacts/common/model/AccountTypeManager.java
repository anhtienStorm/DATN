...
class AccountTypeManagerImpl extends AccountTypeManager
    ...
    protected void loadAccountsInBackground() {
        ...
        synchronized (this) {
            ...
            mAccounts = checkAddLocalAccountIfNeed(allAccounts); // TrungTH them vao
            mContactWritableAccounts = checkAddLocalAccountIfNeed(contactWritableAccounts);
    ...
    // TrungTH
    protected List<AccountWithDataSet> checkAddLocalAccountIfNeed(List<AccountWithDataSet> accounts) {
        // Bkav HuyNQN tren ca ban VN va My deu xu ly cho phep luu vao bo nho may neu nhu khong co acc google
        if(/*Utils.isMyanmar() &&*/ accounts != null && accounts.size() > 0){
            for(AccountWithDataSet account : accounts){
                if(account.type.contains(AccountsListAdapter.ACC_GOOGLE)){
                    return accounts;
                }
            }
            AccountWithDataSet localAccount = new AccountWithDataSet(mContext.getString(R.string.keep_local) , null, null);
