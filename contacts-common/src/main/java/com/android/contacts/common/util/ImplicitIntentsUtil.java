...
public class ImplicitIntentsUtil {
    ...
    public static void startActivityInApp(Context context, Intent intent) {
        ...
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);//Bkav QuangNDb them vao de rom 5.1 khong bi loi
        context.startActivity(intent);
    }

    // Bkav TienNAb: tao ham start activity rieng cho giao dien QuickContact Activity
    public static void startActivityQuickContactInApp(Context context, Intent intent) {
        String packageName = context.getPackageName();
        intent.setPackage(packageName);
