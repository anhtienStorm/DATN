...
public class GeoUtil {
    ...
    public static String getGeocodedLocationFor(Context context,  String phoneNumber) {
        ...
        try {
            ...
            // Bkav HuyNQN se chi hien quoc gia chu khong hien khu vuc nho trong quoc gia nua
            /*return geocoder.getDescriptionForNumber(structuredPhoneNumber, locale);*/
            return getCountryNameForNumber(structuredPhoneNumber, locale);
        } catch (NumberParseException e) {
            return null;
        }
    }

    // Bkav HuyNQN lay ten quoc gia theo ngon ngu và ma quoc gia cua so dien thoai
    private static String getCountryNameForNumber(Phonenumber.PhoneNumber number, Locale language) {
        final PhoneNumberUtil phoneUtil = PhoneNumberUtil.getInstance();
        String regionCode = phoneUtil.getRegionCodeForNumber(number);
        return getRegionDisplayName(regionCode, language);
    }

    // Bkav HuyNQN tra ve ten hien thi cua quoc gia da cho
