...
public class PinnedHeaderListView extends AutoScrollListView
    ...
    // Bkav HienDTk: check su kien bam vao view an bam phim trong callog search
    public static boolean isCheckKeyDown ;

    /*private*/protected static final class PinnedHeader {
        View view;
        boolean visible;
        public int y;
        int height;
    ...
    public void setFadingHeader(int viewIndex, int position, boolean fade) {
        ...
        if (fade) {
            ...
            //AnhNDd: custom
            hookToChangeSetFade(bottom,top,headerHeight,header);
        }
    }
    ...
    /**
     * AnhNDd: Tính lại việc làm mờ view đi khi nào
     */
    public void hookToChangeSetFade(int bottom, int top, int headerHeight, PinnedHeader header ){
        if (bottom < headerHeight) {
            int portion = bottom - headerHeight;
            header.alpha = MAX_ALPHA * (headerHeight + portion) / headerHeight;
            header.y = top + portion;
