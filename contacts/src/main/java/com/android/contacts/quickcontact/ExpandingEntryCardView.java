...
public class ExpandingEntryCardView extends CardView {
    ...
    public static final class Entry {
        ...
        private final String mPathAudio;
        private final String mCallHour;

        // Bkav TienNAb: them bien de check khi nao thi sua mau icon giong mau avatar
        private final boolean mShouldApplyColorAlternateIcon;
        private final boolean mShouldApplyColorThirdIcon;
        ...
        public Entry(int id, Drawable mainIcon, String header, String subHeader,
                ...
                int iconResourceId, /*HuyNQn them truong*/ String pathAudio, boolean shouldApplyColorAlternateIcon, boolean shouldApplyColorThirdIcon, String callHour) {
            mId = id;
            mIcon = mainIcon;
            mHeader = header;
            ...
            mPathAudio=pathAudio;
            mShouldApplyColorAlternateIcon = shouldApplyColorAlternateIcon;
            mShouldApplyColorThirdIcon = shouldApplyColorThirdIcon;
            mCallHour = callHour;
        }
        ...
        //Bkav QuangNDb sua thanh public de BtalkQuickContactActivity dung
        ...
        boolean shouldApplyColorAlternateIcon(){
            return  mShouldApplyColorAlternateIcon;
        }

        boolean shouldApplyColorThirdIcon(){
            return  mShouldApplyColorThirdIcon;
        }
    ...
    // Anhdts dich sang trai de khong bi tran ra ngoai le
    @Override
    public boolean showContextMenuForChild(View originalView, float x, float y) {
        DisplayMetrics displaymetrics = getResources().getDisplayMetrics();
        if (displaymetrics.widthPixels - x < 720) {
            x = displaymetrics.widthPixels - 720;
        }
            ...
            if (mEntries != null) {
                for (List<Entry> entryList : mEntries) {
                    for (Entry entry : entryList) {
                        ...
                        if (alternateIcon != null && entry.shouldApplyColorAlternateIcon()) {
                            alternateIcon.mutate();
                            alternateIcon.setColorFilter(mThemeColorFilter);
                        }
                        Drawable thirdIcon = entry.getThirdIcon();
                        if (thirdIcon != null && entry.shouldApplyColorThirdIcon()) {
                            thirdIcon.mutate();
    ...
    private View createEntryView(LayoutInflater layoutInflater, final Entry entry,
        ...
        // Anhdts bind icon sim
        // Nhung so khong co trong danh ba entry =-1
        if (entry.mIconResourceId != 0/* && entry.mId != -1*/) {
            updateSimIcon(icon, entry.mShouldApplyColor);
        }
        ...
        if (!TextUtils.isEmpty(num)) {
            // Bkav TienNAb: format sdt truoc khi hien thi
            header.setText(HelpUtils.isPhoneNumber(num) ? HelpUtils.formatNumber(num) : num);
        ...
        final TextView callHour = (TextView) view.findViewById(R.id.call_hour);
        if (!TextUtils.isEmpty(entry.getText())) {
            text.setText(entry.getText());
            callHour.setText(entry.getCallHour());
        } else {
            callHour.setVisibility(View.GONE);
        ...
        onSetUpPlayAudioView(view,entry.getPathAudio());
    ...
    public void setTitle(String title) {
        ...
        // Bkav TienNAb: bo divider duoi tieu de theo thiet ke moi
...
//                View.GONE : View.VISIBLE);
    ...
    public int getLayoutCardView() {
        return R.layout.expanding_entry_card_item;
    }
