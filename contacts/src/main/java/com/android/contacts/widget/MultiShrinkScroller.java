...
public class MultiShrinkScroller extends FrameLayout {
    ...
    public MultiShrinkScroller(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mMinimumHeaderHeight = initMinimumHeaderHeight(); // TrungTH cong them chieu cao cua status bbar
    ...
    public void initialize(MultiShrinkScrollerListener listener, boolean isOpenContactSquare,
        ...
        mListener = listener;
        mIsOpenContactSquare = forceOpenContactSquareToTrue(isOpenContactSquare); // TrungTh them vao
        ...
        setPaddingForToolBar(); // TrungTH them vao
    ...
    public void setTitle(String title, boolean isPhoneNumber) {
        ...
        if (isPhoneNumber) {
            ...
            if (HelpUtils.isPhoneNumber(title)){
                // Bkav TienNAb: format sdt hien thi
                title = HelpUtils.formatNumber(title);
            }
        }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    if (shouldDismissOnScroll()) {
                        // Bkav TienNAb: chan crash app khi vuot tu tren xuong trong giao dien QuickContactActivity
//                        scrollOffBottom();
                    }
...
//            snapToBottomOnDragFinished();
    ...
    public void scrollOffBottom() {
        ...
        ObjectAnimator translateAnimation = newAniamtionClose(); // TrungTH
    ...
    public void scrollUpForEntranceAnimation(boolean scrollToCurrentPosition) {
        ...
        final ObjectAnimator animator = newAnimationOpen();// TrungTH
    ...
    private void setInterpolatedTitleMargins(float x) {
        ...
        titleLayoutParams.topMargin = caclulateTopMargin(toolbarLayoutParams, pretendBottomMargin, x);
    ...
    private int initMinimumHeaderHeight() {
        return mActionBarSize + getStatusBarHeight();
    }


    private boolean forceOpenContactSquareToTrue(boolean isOpenContactSquare) {
        return true/*isOpenContactSquare*/; // Luc nao cung full man hinh
    }

    private  void setPaddingForToolBar(){
        // TrungTH them padding cho toolbar dung bang statusbar
        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        final FrameLayout.LayoutParams toolbarLayoutParams
                = (FrameLayout.LayoutParams) toolbar
                .getLayoutParams();
    ...
    @NonNull
    ObjectAnimator newAniamtionClose() {
        mScroller.forceFinished(true);
        ObjectAnimator translateAnimation = ObjectAnimator.ofFloat(this, "translationX",
                0, getScreenWidth());
        translateAnimation.setRepeatCount(0);
        translateAnimation.setInterpolator(mPathInterpolator);
    ...
    private int caclulateTopMargin(LinearLayout.LayoutParams toolbarLayoutParams, int pretendBottomMargin, float x) {
        return toolbarLayoutParams.height - pretendBottomMargin
                - mMaximumHeaderTextSize + (int)(getStatusBarHeight()*(1-x));
    }
    ...
    public void fakeNavigationBar(int navHeight) {
        mScrollViewChild.setPadding(mScrollViewChild.getPaddingLeft(),
                mScrollViewChild.getPaddingTop(), mScrollViewChild.getPaddingRight(), 0);
    }
}
