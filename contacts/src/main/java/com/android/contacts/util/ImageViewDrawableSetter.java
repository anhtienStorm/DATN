...
public class ImageViewDrawableSetter {
    ...
    public Bitmap setupContactPhoto(Contact contactData, ImageView photoView, String conversationId) {
    ...
    private Drawable defaultDrawable(Context c, Account account) {
        ...
        if (TextUtils.isEmpty(mContact.getLookupKey())) {
            // HienDTk: neu khong co conversation thi cap nhat mau theo ten hien thi
            if(mConversationId != null){
                request = new DefaultImageRequest(null, mConversationId, contactType,
                        false /* isCircular */);
            }else {
                String displayName = HelpUtils.formatNumber(mContact.getDisplayName());
                request = new DefaultImageRequest(null, displayName, contactType,
                        false /* isCircular */);
            }
