...
public class CompactKindSectionView extends LinearLayout {
    ...
    /*private*/protected ViewGroup mEditors;
    ...
    public void setState(KindSectionDataList kindSectionDataList,
        ...
        if (dataKind != null) {
            // Anhdts doi lai cac icon
            mIcon.setImageDrawable(getMimeTypeDrawable(getContext(),
                    dataKind.mimeType));
    ...
    private void rebuildFromState(ValuesDelta primaryValuesDelta) {
        ...
        for (KindSectionData kindSectionData : mKindSectionDataList) {
            if (StructuredName.CONTENT_ITEM_TYPE.equals(mimeType)) {
                ...
                // Anhdts set gia tri cuoi de hien icon add truong
                int i = 0;
                for (ValuesDelta valuesDelta : kindSectionData.getVisibleValuesDeltas()) {
                    i++;
                    if (i == kindSectionData.getVisibleValuesDeltas().size()) {
                        valuesDelta.setLastView(true);
                    } else {
                        valuesDelta.setLastView(false);
                    }
    ...
    private void addNameEditorViews(AccountType accountType,
        ...
        //AnhNDd: custom
        final StructuredNameEditorView nameView = hookToInfalteStructuredNameEditorView();
    ...
    private void addGroupEditorView(RawContactDelta rawContactDelta, DataKind dataKind) {
        final GroupMembershipView view = (GroupMembershipView) mLayoutInflater.inflate(
                getLayoutMembership(), mEditors, /* attachToRoot =*/ false);
    ...
    private View addNonNameEditorView(RawContactDelta rawContactDelta, DataKind dataKind,
        ...
        // Inflate the layout
        // Anhdts sua lai layout
        final View view = mLayoutInflater.inflate(
                getResourceId(dataKind.mimeType), mEditors, false);
        boolean email = true;
    ...
    public void updateEmptyEditors(boolean shouldAnimate) {
        ...
        if (isNameKindSection) {
            ...
            // Anhdts neu khong co thi hien len
            if (editorCount == emptyEditors.size()) {
                setAcceptShowEmptyField(true);
            }
            setVisibility(VISIBLE);
    ...
    protected void updateEmptyNonNameEditors(boolean shouldAnimate) {
        ...
        if (mShowOneEmptyEditor && getIsAcceptShowEmptyEditor()) {
            final String mimeType = mKindSectionDataList.getMimeType();
    ...
    public int getIdPhoneticView() {
        return R.layout.phonetic_name_editor_view;
    }

    // Anhdts doi layout group
    public int getLayoutMembership() {
        return R.layout.item_group_membership;
    }
}
