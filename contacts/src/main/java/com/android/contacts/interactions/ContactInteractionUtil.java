...
public class ContactInteractionUtil {
    ...
    // Bkav HuyNQN format lay ra gio thuc hien cuoc goi
    @NeededForTesting
    public static String formatHourStringFromTimestamp(long timestamp, Context context) {
        return formatHourStringFromTimestamp(timestamp, context, Calendar.getInstance());
    }
    // Bkav HuyNQN thuc hien lay ra gio thuc hien cuoc goi
    @NeededForTesting
    private static String formatHourStringFromTimestamp(long timestamp, Context context,
                                                        Calendar compareCalendar){
        Calendar interactionCalendar = Calendar.getInstance();
        interactionCalendar.setTimeInMillis(timestamp);

        // compareCalendar is initialized to today return null
        if (compareCalendarDayYear(interactionCalendar, compareCalendar)) {
            return null;
        }
        return DateFormat.getTimeInstance(DateFormat.SHORT).format(
