...
public class ContactDeletionInteraction extends Fragment
    ...
    private void showDialog(int messageId, int positiveButtonId, final Uri contactUri) {
        ...
        mDialog.show();
        //HienDTk: dong nhat mau text cho dialog
        int colorDialog = mContext.getResources().getColor(R.color.btalk_color_bg_actionbar);
        Button nbutton = mDialog.getButton(DialogInterface.BUTTON_NEGATIVE);
        nbutton.setTextColor(colorDialog);
        Button pbutton = mDialog.getButton(DialogInterface.BUTTON_POSITIVE);
        pbutton.setTextColor(colorDialog);
    }
