...
public class HeaderEntryContactListAdapter extends DefaultContactListAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (position == 0 && getHeaderEntryCount() > 0) {
            ...
            itemView.setDrawableResourceHeaderItem(R.drawable.ic_search_add_contact_new);
            itemView.setDisplayName(getContext().getResources().getString(
                    R.string.header_entry_contact_list_adapter_header_title));

            // Bkav HuyNQN fix loi khong the click duoc vao item nay
            itemView.setDescendantFocusability(ViewGroup.FOCUS_BLOCK_DESCENDANTS);
            //Bkav QuangNDb sua lai mau text color header cho giong thiet ke
            itemView.setTextColorHeaderItem();
            return itemView;
        }
