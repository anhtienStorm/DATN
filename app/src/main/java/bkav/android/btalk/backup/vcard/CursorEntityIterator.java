...
public abstract class CursorEntityIterator implements EntityIterator {
	...
	/**
	 * Constructor that makes initializes the cursor such that the iterator
	 * points to the first Entity, if there are any.
	 * 
	 * @param cursor
	...
	/**
	 * Returns the entity that the cursor is currently pointing to. This must
	 * take care to advance the cursor past this entity. This will never be
	 * called if the cursor is at the end.
	 * 
	 * @param cursor
	 *            the cursor that contains the entity rows
	 * @return the entity that the cursor is currently pointing to
	 * @throws RemoteException
	 *             if a RemoteException is caught while attempting to build the
	 *             Entity
	 */
	...
	public void remove() {
		throw new UnsupportedOperationException(
				"remove not supported by EntityIterators");
	}

	/* (non-Javadoc)
	 * @see bms.backup.EntityIterator#reset()
	 */
