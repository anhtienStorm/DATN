...
/**
 * Created by quangnd on 18/08/2017.
 */
...
public abstract class CursorRecyclerViewAdapter <VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH>{
    ...
    public void onBindViewHolder(VH viewHolder, int position) {
        ...
        if(!mCursor.isClosed()) { // Bkav HuyNQN fix loi crash do onclick cursor dong truoc do
    ...
    private Cursor swapCursor(Cursor newCursor, SparseIntArray changes) {
        ...
        if (mCursor != null) {
            ...
            String[] column = newCursor.getColumnNames();
            if (Arrays.asList(column).contains(COLUMN_ID)) {
                mRowIdColumn = newCursor.getColumnIndexOrThrow(COLUMN_ID);
            }
            mDataValid = true;
        } else {
            mRowIdColumn = -1;
