...
public class Contact {
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
        ...
        if (!MmsSmsUtils.isEmailAddress(number)) {
            formattedNumber = PhoneNumberUtils.formatNumber(number, numberE164, GeoUtil.getCurrentCountryIso(Factory.get().getApplicationContext()));
        }
    ...
    public synchronized void setNumber(String number) {
        if (!MmsSmsUtils.isEmailAddress(number)) {
            mNumber = PhoneNumberUtils.formatNumber(number, mNumberE164, GeoUtil.getCurrentCountryIso(Factory.get().getApplicationContext()));
        } else {
            mNumber = number;
        }
    ...
    public synchronized boolean isEmail() {
        return MmsSmsUtils.isEmailAddress(mNumber);
    }
    ...
    private static class ContactsCache {
        ...
        private static final String[] CALLER_ID_PROJECTION = new String[] {
                ...
                ContactsContract.CommonDataKinds.Phone.PHOTO_URI, //9
                ContactsContract.CommonDataKinds.Phone.LOOKUP_KEY //10
        };
        ...
        private static final int PHONE_PHOTO_URI = 9;
        ...
        private Contact getContactInfo(Contact c) {
            ...
            } else if (MmsSmsUtils.isEmailAddress(c.mNumber)) {
        ...
        private boolean isAlphaNumber(String number) {
            ...
            if (MmsSmsUtils.isAlias(number,ParticipantData.DEFAULT_SELF_SUB_ID)) {
                return true;
            }
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)) {
                ...
                // Anhdts doi lai
                String numberE164 = DialerCompatUtils.formatNumberE164(number, GeoUtil.getCurrentCountryIso(Factory.get().getApplicationContext()));
                String selection;
                String[] args;
        ...
        private void fillPhoneTypeContact(final Contact contact, final Cursor cursor) {
            synchronized (contact) {
                ...
                contact.mPhotoUri = cursor.getString(PHONE_PHOTO_URI);
                contact.mLookupKey = cursor.getString(PHONE_LOOKUP_KEY);
                Log.d(TAG, "fillPhoneTypeContact: "+cursor.getString(PHONE_PHOTO_URI));
                Log.d(TAG, "fillPhoneTypeContact: "+cursor.getString(PHONE_LOOKUP_KEY));
                if (Log.isLoggable(TAG, Log.DEBUG)) {
        ...
        private Contact internalGet(String numberOrEmail, boolean isMe) {
            synchronized (ContactsCache.this) {
                ...
                final boolean isNotRegularPhoneNumber = isMe || MmsSmsUtils.isEmailAddress(numberOrEmail) ||
                        MmsSmsUtils.isAlias(numberOrEmail,ParticipantData.DEFAULT_SELF_SUB_ID);
        ...
        private void remove(Contact contact) {
            synchronized (ContactsCache.this) {
                ...
                final boolean isNotRegularPhoneNumber = contact.isMe() ||
                        MmsSmsUtils.isEmailAddress(number) ||
                        MmsSmsUtils.isAlias(number, ParticipantData.DEFAULT_SELF_SUB_ID);
                final String key = isNotRegularPhoneNumber ?
