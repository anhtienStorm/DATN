...
public class FilteredNumberProvider extends ContentProvider {
    ...
    private static final int FILTERED_SMARTDIAL_TABLE = 4;
    ...
    public boolean onCreate() {
        ...
        // Bkav HuyNQN uri query toi smartdialer_table
        sUriMatcher.addURI(ObjectFactory.getFilteredNumberProviderAuthority(),
                DialerDatabaseHelper.Tables.SMARTDIAL_TABLE,
                FILTERED_SMARTDIAL_TABLE);
        return true;
    }
...
//        qb.setTables(DialerDatabaseHelper.Tables.FILTERED_NUMBER_TABLE);
        ...
        switch (match) {
            case FILTERED_NUMBERS_TABLE:
                qb.setTables(DialerDatabaseHelper.Tables.FILTERED_NUMBER_TABLE);
                break;
            case FILTERED_SMARTDIAL_TABLE: // Bkav HuyNQN thuc hien select smartdialer_table
                qb.setTables(DialerDatabaseHelper.Tables.SMARTDIAL_TABLE);
                break;
    ...
    private void setDefaultValues(ContentValues values) {
        ...
        if (values.getAsInteger(FilteredNumberColumns.TIMES_FILTERED) == null) {
            values.put(FilteredNumberColumns.TIMES_FILTERED, 0);
        }
    ...
    private String getSelectionWithId(String selection, long id) {
        if (TextUtils.isEmpty(selection)) {
            return FilteredNumberColumns._ID + "=" + id;
        } else {
            return selection + "AND " + FilteredNumberColumns._ID + "=" + id;
        }
    }
