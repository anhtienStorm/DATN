...
public class DialerDatabaseHelper extends SQLiteOpenHelper {
    protected static final String TAG = "DialerDatabaseHelper";
    protected static final boolean DEBUG = false;
    private boolean mIsTestInstance = false;

    protected static DialerDatabaseHelper sSingleton = null;
     ...
     * </pre>
     * <p>
     * Bkav QuangLH: tang len 10
     * Anhdts 11 them cot full_name
     * Anhdts 12 them cot priority
     * Anhdts 13 them cot ten ma hoa
     */
     ...
     */
    protected static final String DATABASE_LAST_CREATED_SHARED_PREF = "com.android.dialer";
    protected static final String LAST_UPDATED_MILLIS = "last_updated_millis";
    private static final String DATABASE_VERSION_PROPERTY = "database_version";

    // Anhdts doi tu 40 den 80
    protected static final int MAX_ENTRIES = 80;
    ...
    //Bkav QuangNDb can them authority da regis manifest tren android O ++
    public static Uri getSmartDialUpdatedUri() {
        Uri.Builder builder = new Uri.Builder().authority("bkav.android.messaging.datamodel.MessagingContentProvider")
                .path(SMART_DIAL_UPDATED_URI.getPath());
    ...
    public interface SmartDialDbColumns {
        ...
        // Anhdts ghi lai so cuoc goi di
        static final String TIMES_OUT_CALL = "time_out_call";
        static final String PREFIX_FULL_NAME = "prefix_full_name";
        static final String PREFIX_PRIORITY = "prefix_priority";
        static final String NORMALIZE_NAME = "normalize_name";
    ...
    public static interface PhoneQuery {
        ...
        static final String[] PROJECTION = new String[]{
                ...
                Phone.PHOTO_ID,                     // 7
                Data.LAST_TIME_CONTACTED,           // 8 TODO TrungTH chuyen sang lastTimeContactted truong last time used khong cho dung du lieu vua moi lien lac
                Data.TIMES_USED,                    // 9
                Contacts.STARRED,                   // 10
                Data.IS_SUPER_PRIMARY,              // 11
                Contacts.IN_VISIBLE_GROUP,          // 12
                Data.IS_PRIMARY,                    // 13
                /* BKav TrungTH, xóa truong nay di add vao ben duoi khi tren android M,
                 Data.CARRIER_PRESENCE,              // 16*/
        ...
        static final int PHONE_ACCOUNT_TYPE = 14; // Bkav TrungTh 15 => 14
        static final int PHONE_ACCOUNT_NAME = 15; // Bkav TrungTh 16 => 15
        static final int PHONE_NORMALIZED_NUMBER = 16; // Anhdts de get truong normalized
        // Bkav TrungTH, dao gia tri PHONE_CARRIER_PRESENCE xuong thanh 16 => cac truong khác day gia tri len
        static final int PHONE_CARRIER_PRESENCE = 17;
     ...
     */
    protected static interface SmartDialSortingOrder {
        /**
    ...
    public static class ContactNumber {
        ...
        // Anhdts du lieu call log khong co lookup key
        public Uri lookupUri;
        public int mCallType;
        public String mCustomLabel;
        public String mThumpUri;

        public static final long ID_DEFAULT_EXCHANGE_CONTACT = 1000001;

        public void setPhoneData(int calType, String label, String thumpUri) {
            mCallType = calType;
            mCustomLabel = label;
            mThumpUri = thumpUri;
        }
        ...
        /**
         * Anhdts them constructor cho so tong dai
         */
        public ContactNumber(String displayName, String phoneNumber, String lookupKey, Uri lookupUri) {
            this.dataId = ID_DEFAULT_EXCHANGE_CONTACT;
            this.id = ID_DEFAULT_EXCHANGE_CONTACT;
            this.displayName = displayName;
            this.phoneNumber = phoneNumber;
            this.lookupKey = lookupKey;
            this.photoId = 0;
            this.carrierPresence = -1;
        ...
        /**
         * Anhdts them constructor cho so o trong call log
         */
        public ContactNumber(long id, long dataID, String displayName, String phoneNumber,
                             String lookupKey, Uri lookupUri, long photoId, int carrierPresence,
                             String accountType, String accountName) {
            this(id, dataID, displayName, phoneNumber, lookupKey, photoId, carrierPresence, accountType, accountName);
            this.lookupUri = lookupUri;
        }
    ...
    private void setupTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + Tables.SMARTDIAL_TABLE + " ("
                ...
                + SmartDialDbColumns.PREFIX_PRIORITY + " TEXT, "     // Anhdts cot text nguoi dung hay search
                + SmartDialDbColumns.NORMALIZE_NAME + " TEXT "     // Anhdts cot ten danh ba bo dau
                + ");");
    ...
    private boolean isNeedUpgradeForSmartSearch() {
        ...
        if (pref != null) {
            // Anhdts khong can upgrade nua, chac la logic cua google ngay xua. Gio minh tao moi bang mac dinh co 2 cot nay roi
            int mSharePrefVersion = pref.getInt(DATABASE_SHAREPREF_KEY, 1);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldNumber, int newNumber) {
        ...
        if (oldVersion < 10) {
            // Bkav QuangLH: Btalk 1 chac cung co table nay. Them code AnhDTs them vao.
            db.execSQL("ALTER TABLE " + Tables.SMARTDIAL_TABLE + " ADD COLUMN "
                    + SmartDialDbColumns.TIMES_OUT_CALL + " INTEGER DEFAULT 0");
        ...
        if (oldVersion < 11) {
            db.execSQL("DROP TABLE IF EXISTS " + Tables.SMARTDIAL_TABLE);
            createDatabaseWithPrefix(db);
            oldVersion = 11;
            resetSmartDialLastUpdatedTime();
        }

        // Anhdts them cot tu khoa hay dung tim kiem voi tung nguoi
        if (oldVersion < 12) {
            setupTables(db);
            oldVersion = 12;
            isOvermuch = true;
        ...
        // ANhdts: them cot normalize name
        // Sua loi duplicate khi nhay tu ban 11 sang 13
        if (oldVersion < 13) {
            restoreFirstUpdate();
            addColumnNormalizeName(db, isOvermuch);
            oldVersion = 13;
        }
    ...
    private void resetSmartDialLastUpdatedTime() {
        ...
        final Editor editor = databaseLastUpdateSharedPref.edit();
    ...
    public void startSmartDialUpdateThread() {
        if (PermissionsUtil.hasContactsPermissions(mContext)) {
            new SmartDialUpdateAsyncTask(this).execute();
        }
    }

    private static class SmartDialUpdateAsyncTask extends AsyncTask<Void, Void, Void> {
        WeakReference<DialerDatabaseHelper> weakReference;

        SmartDialUpdateAsyncTask(DialerDatabaseHelper outer) {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            if (weakReference != null) {
                weakReference.get().updateSmartDialDatabase();
            }
            return null;
        }
    ...
    protected void insertUpdatedContactsAndNumberPrefix(SQLiteDatabase db,
                                                        Cursor updatedContactCursor, Long currentMillis, String lastUpdateMillis) {
        db.beginTransaction();
        try {
            final String sqlInsert = "INSERT INTO " + Tables.SMARTDIAL_TABLE + " (" +
                    ...
                    SmartDialDbColumns.NORMALIZE_NAME + ") " +
                    " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            while (updatedContactCursor.moveToNext()) {
                ...
                // Anhdts getContactId
                long contactId = updatedContactCursor.getLong(
                ...
                if (displayName == null) {
                    ...
                    // them check neu ten lien he la sdt thi bo dau cach khi format sdt di
                    if (TextUtils.isDigitsOnly(displayName.replaceAll("\\s",""))){
                        insert.bindString(5, displayName.replaceAll("\\s",""));
                    } else {
                ...
                insert.bindLong(13, getPhoneCarriedPresence(updatedContactCursor)); // Bkav TrungTH them ham moi
                ...
                // Anhdts bind lai truong tong cuoc goi va cuoc goi di
                if (getOutCallTimes(db, updatedContactCursor, insert, number, lastUpdateMillis)) {
                    insert.bindLong(7, updatedContactCursor.getLong(PhoneQuery.PHONE_LAST_TIME_USED));
                } else {
                    long lastTime = (mContactsRemove != null && mContactsRemove.get(contactId) != null)
                            ? mContactsRemove.get(contactId).getLastTimeUsed() : updatedContactCursor.getLong(
                            PhoneQuery.PHONE_LAST_TIME_USED);
                    insert.bindLong(7, lastTime);
                }
                getPrefixFullName(insert, displayName);
                getPrefixPriority(insert, displayName, contactId);
                getNormalizeName(insert, displayName);
                ...
                for (String numberPrefix : numberPrefixes) {
                    ...
                    numberInsert.bindString(2, ENCRYPT_PREFIX_NUMBER + numberPrefix + ENCRYPT_PREFIX_NUMBER);
        ...
        try {
            ...
            while (nameCursor.moveToNext()) {
                ...
                if (mMultiGetNameNumberMethod != null) {
                    ...
                    // Anhdts loc bo cac gia tri khong can thiet va trung nhau,
                    // search va them vao du lieu se nhanh hon, khoi tao du lieu dau nhanh hon
    ...
    private void updateSmartDialDatabase() {
        initMultiLanguageSearch(); //TODO Bkav TrungTH Anhdts xem lai
        // Bkav HienDTk: fix bug: java.lang.Throwable: java.lang.RuntimeException: An error occurred while executing doInBackground() => BOS-2522 - Start
        try {
            ...
            synchronized (mLock) {
                ...
                removeDeletedContacts(db, getDeletedContactCursor(lastUpdateMillis), Long.parseLong(lastUpdateMillis));
                ...
                if (!lastUpdateMillis.equals("0")) {
                    ...
                    if (DEBUG) {
                        stopWatch.lap("Finished updatedContactCursor count: " + updatedContactCursor.getCount());
                    }
                    try {
                        removeUpdatedContacts(db, updatedContactCursor);
                        if (DEBUG) {
                            stopWatch.lap("Finished removeUpdatedContacts");
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    } finally {
                ...
                if (updatedPhoneCursor == null) {
                    Log.e(TAG, "SmartDial query receiveadb pull /data/data/bkav.android.btalk/databases/dialer.dbd null for cursor");
                    return;
                }

                try {
                    /** Inserts recently updated phone numbers to the smartdial database.*/
                    insertUpdatedContactsAndNumberPrefix(db, updatedPhoneCursor, currentMillis, lastUpdateMillis); // Anhdts them truong lan cuoi cung cap nhat
                    if (DEBUG) {
                ...
                db.execSQL("CREATE INDEX IF NOT EXISTS smartdial_sort_index ON " +
                        ...
                        SmartDialDbColumns.CONTACT_ID + ", " +
                        SmartDialDbColumns.IS_PRIMARY + ", " +
                        SmartDialDbColumns.TIMES_OUT_CALL + ", " +//Anhdts
                        SmartDialDbColumns.PREFIX_FULL_NAME +//Anhdts
                        ");");
                ...
                // Anhdts update callLog
                if (mIsFirstRunOrCleanDB) {
                    updateSmartCallLog("0", db, currentMillis);
                } else {
                    updateSmartCallLog(lastUpdateMillis, db, currentMillis);
                }
                // Anhdts xoa du lieu query
                clearRecentQuery();

                final Editor editor = databaseLastUpdateSharedPref.edit();
                editor.putLong(LAST_UPDATED_MILLIS, currentMillis);
                editor.apply();
                // Anhdts: set du lieu da duoc khoi tao
                setFirstUpdateComplete();
                //Bkav QuangNDb update fix target sdk 28
                // Notify content observers that smart dial database has been updated.
                mContext.getContentResolver().notifyChange(getSmartDialUpdatedUri(), null, false);
            }
        }catch (SQLiteReadOnlyDatabaseException e){
            e.printStackTrace();
        }

// Bkav HienDTk: fix bug: java.lang.Throwable: java.lang.RuntimeException: An error occurred while executing doInBackground() => BOS-2522 - End
    ...
    public ArrayList<ContactNumber> getLooseMatches(String query,
        ...
        if (inUpdate || query.length() == 0) {
            // Anhdts neu dang update thi xoa gia tri tim kiem di
            clearSuggestValue();
        ...
        // Anhdts remake cursor
        final Cursor cursor;
        if (isConfigBtalk()) {
            cursor = getBtalkCursor(db, currentTimeStamp, query);
        } else {
            String sql = "SELECT " +
                    SmartDialDbColumns.DATA_ID + ", " +
                    SmartDialDbColumns.DISPLAY_NAME_PRIMARY + ", " +
        ...
        // Anhdts
        if (!processPrefixBtalk(cursor, stopWatch, nameMatcher, query, result)) {
            try {
                if (DEBUG) {
    ...
    // ANhdts bind so cuoc goi di va tong so cuoc goi
    ...
    //Bkav TrungTH Add them truong CARRIER_PRESENCE voi android tren M
    static {
        List<String> projectionList = Lists.newArrayList(PhoneQuery.PROJECTION);
        if (CompatUtils.isMarshmallowCompatible()) {
            projectionList.add(Data.CARRIER_PRESENCE);
        }
        PROJECTION_COLUMNS = projectionList.toArray(new String[projectionList.size()]);
    }

    //Bkav TrungTH Check tren android M  moi get gia tri, con khong thi tra ve 0
    private int getPhoneCarriedPresence(Cursor updatedContactCursor) {
        if (updatedContactCursor.getColumnIndex(Data.CARRIER_PRESENCE) != -1) {
            return updatedContactCursor.getInt(PhoneQuery.PHONE_CARRIER_PRESENCE);
        }
        return 0;
    }
     ...
     */
    protected boolean processPrefixBtalk(Cursor cursor, StopWatch stopWatch, SmartDialNameMatcher nameMatcher, String query, ArrayList<ContactNumber> result) {
        return false;
    }

    /**
     * Anhdts luu lai mang cuoc goi di vua moi xoa
     */
    protected LongSparseArray<ContactRemove> mContactsRemove;

    /**
     * Anhdts du lieu da duoc khoi tao lan dau tien vao app hoac sau khi xoa du lieu
     */
    protected void setFirstUpdateComplete() {
    }

    /**
     * Anhdts khoi phuc lai du lieu
     */
    ...
    /**
     * Anhdts upgrade database them cot convert ten sang sdang magic pad
     */
    ...
    public static final String ENCRYPT_PREFIX_NUMBER = "*";

    /**
     * Anhdts them cot prefix hay search
     */
    protected void addColumnNormalizeName(SQLiteDatabase db, boolean b) {
    }
    ...
    public class ContactRemove {
        ...
        private final int mCallUsed;

        private final String mPrefix;

        private final long mDataIdPriority;

        private final long mLastTimeUsed;

        public ContactRemove(int callOut, int callUsed, String prefix, long priority, long lastTimeUsed) {
            mCallOut = callOut;
            mCallUsed = callUsed;
            mPrefix = prefix;
            mDataIdPriority = priority;
        ...
        public long getDataIdPriority() {
            return mDataIdPriority;
        }
    ...
    /**
     * Anhdts sinh id cho so dien thoai
     */
    ...
    public class SmartUpdate {
        public static final int DATA_ID_COLUMN = 1;
        public static final int NUMBER_COLUMN = 2;
        public static final int CONTACT_ID_COLUMN = 3;
        public static final int LOOKUP_KEY_COLUMN = 4;
        public static final int DISPLAY_NAME_PRIMARY_COLUMN = 5;
        public static final int PHOTO_ID_COLUMN = 6;
        public static final int LAST_TIME_USED_COLUMN = 7;
        public static final int COUNT_USED_COLUMN = 8;
        public static final int STARRED_COLUMN = 9;
        public static final int IS_SUPER_PRIMARY_COLUMN = 10;
        public static final int IN_VISIBLE_GROUP_COLUMN = 11;
        public static final int IS_PRIMARY_COLUMN = 12;
        public static final int CARRIER_PRESENCE_COLUMN = 13;
        public static final int LAST_SMARTDIAL_UPDATE_TIME_COLUMN = 14;
        public static final int ACCOUNT_TYPE_COLUMN = 15;
        public static final int ACCOUNT_NAME_COLUMN = 16;
        public static final int COUNT_OUT_CALL_COLUMN = 17;
        public static final int PREFIX_FULL_NAME_COLUMN = 18;
        public static final int PREFIX_PRIORITY_COLUMN = 19;
        public static final int NORMALIZE_NAME_COLUMN = 20;
    }

    protected boolean mIsFirstRunOrCleanDB;

    // Anhdts check cac so o trong callLog
    protected void updateSmartCallLog(String lastUpdateMillis, SQLiteDatabase db, long currentMillis) {
