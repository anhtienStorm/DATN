...
public class SmartDialPrefix {
    ...
    public static class PhoneNumberTokens {
        ...
        public final int countryCodeOffset;
    ...
    public static ArrayList<String> parseToIndexTokens(String contactName) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            char s = contactName.charAt(i);
            if (mMap.isValidDialpadCharacter(c)) {
                // them dieu kien de cat chuoi khi gap ky tu in hoa
                if (isUperCaseCharacter(s)){
                    d++;
                    if (d == 2) {
    ...
    // Bkav TienNAb: ham check xem co phai ky tu in hoa hay khong
    public static boolean isUperCaseCharacter(char c){
        convertCharacterBtalk(c);
        for (char i = 'A'; i <= 'Z'; i++){
            if (i == c)
                return true;
        }
    ...
    // Bkav TienNAb: ham convert tu ky tu tieng viet sang ky tu tieng anh
    private static char convertCharacterBtalk(char key) {
        switch (key) {
            case 'ắ':
            case 'ằ':
            case 'ặ':
    ...
    public static ArrayList<String> parseToNumberTokens(String number) {
        ...
        if (!TextUtils.isEmpty(number)) {
            ...
            if (phoneNumberTokens.countryCodeOffset != 0) {
                // Anhdts sau khi format thi bo duoc ma vung
                // nhung khong co so 0 o dau nen can them 0 vao cho dung format so dien thoai
                result.add("0" + SmartDialNameMatcher.normalizeNumber(number,
