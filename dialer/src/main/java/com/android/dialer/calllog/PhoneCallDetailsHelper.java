...
public class PhoneCallDetailsHelper {
    ...
    protected static final int MAX_CALL_TYPE_ICONS = 3;
    ...
    public void setPhoneCallDetails(PhoneCallDetailsViews views,
        ...
        // Gop icon cuoc goi va video vao lam 1
...
//                (details.features & Calls.FEATURES_VIDEO) == Calls.FEATURES_VIDEO);
        ...
        if (!TextUtils.isEmpty(filter) && phoneNum.contains(filter)) {
            ...
            // Bkav HuyNQN tach code cu ra thanh ham con getStyle()
            displayNumber = getStyle(start,end,phoneNum);
        }
        if (TextUtils.isEmpty(details.getPreferredName())) {
            ...
            if (!TextUtils.isEmpty(filter) && nameText.toString().toUpperCase()
                ...
                // Bkav HuyNQN tach code cu ra thanh ham con getStyle()
                nameText = getStyle(start, end, nameText);
            }
        }
        // Bkav HienDTk: set text sdt
        views.numberView.setText(displayNumber);
        ...
        if (!customViewSecondary(views, details)) {
            // Bold if not read
        ...
        // Anhdts
        // Bkav HienDTk: neu dang search thi khong cho setNumberOrSpamView de boi vang sdt
        if(TextUtils.isEmpty(filter)){
            setNumberOrSpamView(views, details);
        }
        setCallCount(views.nameView, count);
        setBkavTypeface(views); // Bkav TrungTh them de set font
    }

    // Bkav HuyNQN ham con getStyle() cho phep lop con ghi de de doi mau chu
     ...
     */
    protected CharSequence getCallLocationAndDate(PhoneCallDetails details) {
    ...
    protected String getVoicemailDuration(PhoneCallDetails details) {
    ...
    /**************************BKAV********************/
    // Anhdts set so dien thoai cho list view call log
    protected void setNumberOrSpamView(PhoneCallDetailsViews view, PhoneCallDetails details) {}

    // Anhdts set so cuoc goi cho list view call log
