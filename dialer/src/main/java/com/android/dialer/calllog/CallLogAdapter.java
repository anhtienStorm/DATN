...
public class CallLogAdapter extends GroupingListAdapter
        ...
        ExtendedBlockingButtonRenderer.Listener, FastScroller.BubbleTextGetter{
    ...
    // Bkav TienNAb: tao bien hashmap luu ngay theo tung vi tri item
    private HashMap<Integer, String> mPositionToDay = new HashMap<>();

    // Bkav TienNAb: tra ve ngay theo tung vi tri item
    @Override
    public String getTextToShowInBubble(int pos) {
        return mPositionToDay.get(pos);
    }
    ...
    protected final ContactInfoHelper mContactInfoHelper;
    protected final VoicemailPlaybackPresenter mVoicemailPlaybackPresenter;
    private final CallFetcher mCallFetcher;
    protected final FilteredNumberAsyncQueryHandler mFilteredNumberAsyncQueryHandler;
    ...
    protected int mCurrentlyExpandedPosition = RecyclerView.NO_POSITION;
    ...
    protected long mCurrentlyExpandedRowId = NO_EXPANDED_LIST_ITEM;
    protected int mHiddenPosition = RecyclerView.NO_POSITION;
    ...
    protected ContactsPreferences mContactsPreferences;

    protected boolean mShowVoicemailPromoCard = false;

    /** Instance of helper class for managing views. */
    protected final CallLogListItemHelper mCallLogListItemHelper;

    /** Cache for repeated requests to Telecom/Telephony. */
    protected final CallLogCache mCallLogCache;

    /** Helper to group call log entries. */
    protected final CallLogGroupBuilder mCallLogGroupBuilder;

    /**
     * The OnClickListener used to expand or collapse the action buttons of a call log entry.
     */
    protected final View.OnClickListener mExpandCollapseListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            ...
            // Anhdts neu trong che do delete thi tick contact
            if (mIsModeDelete) {
                viewHolder.setChooseDelete(CallLogAdapter.this);
                return;
            }

            // Anhdts bo expandActionHolder thay bang cuoc goi
            if (getActionCallBkav(viewHolder)) {
                return;
            }
     ...
     */
    protected AccessibilityDelegate mAccessibilityDelegate = new AccessibilityDelegate() {
        @Override
    ...
    public CallLogAdapter(
        ...
        // Anhdts
        mCallLogCache = getCallLogCache(mContext.getApplicationContext());

        // Anhdts
        PhoneCallDetailsHelper phoneCallDetailsHelper = createPhoneCallDetailsHelper(resources);
    ...
    public void onBlockedNumber(String number,String countryIso) {
        String cacheKey = DialerCompatUtils.formatNumberE164(number, countryIso);
     ...
     */
    protected ViewHolder createCallLogEntryViewHolder(ViewGroup parent) {
    ...
    protected void bindCallLogListViewHolder(ViewHolder viewHolder, int position) {
        ...
        // Bkav TienNAb: fomat lai cac dau so moi (03, 08....)
        if (formattedNumber != null){
            formattedNumber = PhoneNumberUtil.formatNumber(formattedNumber.toString());
        }
        ...
        // Anhdts
        views.setModeDelete(mIsModeDelete);
        views.info = info;
        ...
        // Bkav TienNAb: luu cac cap gia tri vi tri, ngay
        mPositionToDay.put(views.getAdapterPosition(),getDayGroupForCall(views.rowId, details.date));

        // Check if the day group has changed and display a header if necessary.
        // Anhdts kiem tra xem co can them header khong
        if (checkSetGroupHeader(c, views, details.date)) {
            views.dayGroupHeader.setVisibility(View.VISIBLE);
            views.dayGroupHeader.setText(getDayGroupForCall(views.rowId, details.date));
            setDividerVisibility(views, View.GONE); // Bkav TrungTH co thoi gian thi hien thi divider
        } else {
            views.dayGroupHeader.setVisibility(View.GONE);
            setDividerVisibility(views, View.GONE); // Bkav TrungTH ko co thoi gian thi hien thi an divider
        }
        ...
        if (mActivityType == ACTIVITY_TYPE_ARCHIVE) {
            ...
            if (details.callTypes[0] == CallLog.Calls.VOICEMAIL_TYPE ||
                    details.callTypes[0] == CallLog.Calls.MISSED_TYPE || details.callTypes[0] == AppCompatConstants.MISSED_IMS_TYPE) {
        ...
        final String spam;
        if (isVietnamese()) {
            spam = c.getString(c.getColumnIndex(CallLogQueryHandler.LABEL_BMS_SPAM_VN));
        }else {
            spam = c.getString(c.getColumnIndex(CallLogQueryHandler.LABEL_BMS_SPAM_ENG));
        }
        if (spam != null && !spam.isEmpty()) {
            details.spamContent = spam;
        }

//        // Bkav HuyNQN Bi lag tai day
        // FIXME: 18/03/2020 chua ro tai sao lai phai goi setPhoneCallDetails 2 lan thi moi khong bi loi mat icon tin nhan
        ...
        views.updatePhoto(CallLogAdapter.this);

        mCallLogListItemHelper.setPhoneCallDetails(views, details, mFilterString);

        // Bkav HuyNQN Them ham lay ra pathrecorder
        setPathRecorder(views, c.getString(c.getColumnIndex(CallLogQueryHandler.RECORD_CALL_DATA)));

        // Bkav HuyNQN xu ly voi viec select item trong callLogRecorder
        setItemCheck(views, c.getLong(CallLogQuery.ID), c.getString(c.getColumnIndex(CallLogQueryHandler.RECORD_CALL_DATA)));
    }

    //Bkav QuangNDb check de ngon ngu tieng viet
    private boolean isVietnamese() {
        Locale locale = mContext.getResources().getConfiguration().getLocales().get(0);
        return "vi_VN".equals(locale.toString());
    }

    // Bkav HuyNQN Lay pathrecorder cai dat cho customview trong than ham
    public void setPathRecorder(CallLogListItemViewHolder view, String path){

    }

    // Bkav HuyNQN set check item trong callLogRecorder
    public void setItemCheck(CallLogListItemViewHolder views, long id, String path){
     ...
     */
    protected int getPreviousDayGroup(Cursor cursor) {
     ...
     */
    protected int getDayGroupForCall(long callId) {
     ...
     */
    protected int[] getCallTypes(Cursor cursor, int count) {
        if (mActivityType == ACTIVITY_TYPE_ARCHIVE) {
     ...
     * @return The features.
     */
    protected int getCallFeatures(Cursor cursor, int count) {
        int features = 0;
        int position = cursor.getPosition();
        for (int index = 0; index < count; ++index) {
     ...
     */
    protected CharSequence getGroupDescription(int group) {
    ...
    //Anhdts Call from child
    protected PhoneCallDetailsHelper createPhoneCallDetailsHelper(Resources resources) {
        return new PhoneCallDetailsHelper(mContext, resources, mCallLogCache);
    }

    // Anhdts kiem tra can them header group khong
    ...
    // Anhdts ham get day group
    protected String getDayGroupForCall(long callId, long date) {
        return String.valueOf(getGroupDescription(getDayGroupForCall(callId)));
    }
    ...
    protected CallLogCache getCallLogCache(Context context) {
        return CallLogCache.getCallLogCache(context);
    }

    protected boolean mIsModeDelete = false;

    protected TreeSet<Long> mListCallDelete;

    protected TreeSet<String> mListVoiceDelete;
    ...
    /**
     * Anhdts kiem tra item do co trong list duoc check khong
     */
    public boolean isCheckDelete(long[] callId, String voiceUri) {
        return (mListCallDelete != null && mListCallDelete.contains(callId[0])) ||
                (mListVoiceDelete != null && voiceUri != null && mListVoiceDelete.contains(voiceUri));
    }
}
