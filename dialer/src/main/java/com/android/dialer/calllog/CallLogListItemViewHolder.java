...
public class CallLogListItemViewHolder extends RecyclerView.ViewHolder
    ...
    protected final BlockNumberDialogFragment.Callback mFilteredNumberDialogCallback;

    protected final int mPhotoSize;
    private ViewStub mExtendedBlockingViewStub;
    protected final ExtendedBlockingButtonRenderer mExtendedBlockingButtonRenderer;

    protected View.OnClickListener mExpandCollapseListener;
    private boolean mVoicemailPrimaryActionButtonClicked;

    protected CallLogListItemViewHolder(
            Context context,
    ...
    private void updatePrimaryActionButton(boolean isExpanded) {
        if (!TextUtils.isEmpty(voicemailUri)) {
            ...
            if (PhoneNumberUtil.canPlaceCallsTo(number, numberPresentation) && !mIsModeDelete) {
                ...
                if (nameOrNumber != null) {
                    primaryActionButtonView.setContentDescription(TextUtils.expandTemplate(
                            ...
                            nameOrNumber));
                    primaryActionButtonView.setImageResource(getImgResActionButton());
                    primaryActionButtonView.setVisibility(View.VISIBLE);
                }
            } else {
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.primary_action_button && !TextUtils.isEmpty(voicemailUri)) {
            ...
            if (intentProvider != null) {
                ...
                if (intent != null) {
                    // Anhdts kiem tra xem co phai gui tin nhan khong
                    if (intent.getAction().equals(Intent.ACTION_SENDTO)) {
                        registerFactoryActionSend(intent);
                    } else {
    ...
    // Anhdts bo cardView su dung view khac de setTag
    public View mBtalkCallLogEntryView;

    // Anhdts override lai ben lop cha
    protected void fixBeforeCallAction() {
        final Intent intent = new Intent(
                Intent.ACTION_DIAL, CallUtil.getCallUri(number));
        // Anhdts change activity goi dien
    ...
    protected IntentProvider getIntentPrimaryButton() {
        return IntentProvider.getReturnCallIntentProvider(number + postDialDigits);
    }
    ...
    protected String mFinalNumber;

    protected String mFinalDisplayNumber;

    protected ContactInfo mFinalInfo;

    protected String mFinalPostDialDigits;

    protected long mFinalRowId;

    protected long[] mFinalCallId;
    ...
    protected boolean mIsModeDelete;

    /**
     * Anhdts action click vao view trong che do xoa hang loat
     */
    ...
    private boolean mIsShowTextViewCallLogRecorder = false; // Bkav HuyNQN  kiem tra xem co them XEM NHAT KY GHI AM CUOC GOI vao context menu hay khong.

    public void setIsShowTextViewCallLogRecorder(boolean isShowTextViewCallLogRecorder) {
        this.mIsShowTextViewCallLogRecorder = isShowTextViewCallLogRecorder;
    }

    public boolean isShowTextViewCallLogRecorder() {
        return mIsShowTextViewCallLogRecorder;
    }

    public boolean mIsSelect; // Bkav HuyNQN kiem tra xem item co duoc chon hay khong
}
