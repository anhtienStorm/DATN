...
public final class SmsReceiver extends BroadcastReceiver {
    ...
    public static void deliverSmsMessages(final Context context, final int subId,
                                          final int errorCode, final SmsMessage[] messages, int slotId) {
        ...
        LogUtil.d(TAG, "deliverSmsMessages: Received");
        ...
        // QuangNDb them doan put slotId vao value message
        messageValues.put("slot", slotId);
        if (OsUtil.isAtLeastL_MR1()) {
            messageValues.put(Sms.SUBSCRIPTION_ID, subId);
        }
        //Bkav QuangNDb them logic iccid
        String iccid = PhoneUtils.getDefault().getDefaultSmsIccid(subId);
        messageValues.put("iccid", iccid);
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if (PhoneUtils.getDefault().isSmsEnabled()) {
            ...
            if (OsUtil.isSecondaryUser() &&
                ...
                // Bkav QuangNDb them intent de xu ly tin nhan den nhan biet dc sim 1, sim2
                postNewMessageSecondaryUserNotification(intent);
    ...
    public static void postNewMessageSecondaryUserNotification(Intent intent) {
        // QuangNDb them doan lay slotId
//        final int slotId = intent.getIntExtra("slot", -1);
        // Anhdts bo icon sim
        // Anhdts tren b1 thi bo ava nguoi va thay bang icon tin nhan
        final int idResSmallIcon;
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP_MR1) {
            idResSmallIcon = R.drawable.ic_sim_small_0;
        } else {
            idResSmallIcon = R.drawable.bkav_icon_mms;
        }
//        if (PhoneUtils.getDefault().getActiveSubscriptionCount() > 1) {// TH 2 SIM
//            if (slotId == 0) {// khe sim 1
//                idResSmallIcon =  R.drawable.ic_sim_small_1;
//            } else if (slotId == 1) {// khe sim 2
//                idResSmallIcon =  R.drawable.ic_sim_small_2;
//            }else {// intent k co du lieu ve sim slot
//                idResSmallIcon =  R.drawable.ic_sim_small_0;
//            }
//        }else {
//            idResSmallIcon =  R.drawable.ic_sim_small_0;
//        }
        ...
        // Bkav QuangNDb doi icon mac dinh ve icon btalk
