 ...
 * a bit of SmsManager.
 *
 * The class instance can only be obtained via the get(int mSlotId) method parameterized
 * by a SIM subscription ID. On pre-L_MR1, the mSlotId is not used and it has to be
 * the default mSlotId (-1).
 *
 * A convenient getDefault() method is provided for default mSlotId (-1) on any platform
 */
public abstract class PhoneUtils {
    private static final String TAG = LogUtil.BUGLE_TAG;

    // Bkav HaiKH - Fix bug BOS-3244- Start
    // Biến check nếu setting của BL01 đặt là hỏi trước khi gửi tin nhắn
    public static final int DEFAULT_SIM_SETTING_ALWAYS_ASK_BL01 = -2;
    ...
    /**
     * Returns the "effective" mSlotId, or the mSlotId used in the context of actual messages,
     ...
     *
     * @param subId The input mSlotId
     * @return the real mSlotId if we can convert
     */
     ...
     *
     * @return the relevant SmsManager instance based on OS version and mSlotId
     */
    ...
    /**
     * Get the default SMS iccid
     *
     * @return the default iccid
     */
    public abstract String getDefaultSmsIccid(int subId);
    ...
    /**
     * For L_MR1, system may return a negative mSlotId. Convert this into our own
     * mSlotId, so that we consistently use -1 for invalid or default.
     *
     * see b/18629526 and b/18670346
     *
     * @param intent The push intent from system
     * @param extraName The name of the sub id extra
     * @return the mSlotId that is valid and meaningful for the app
     */
     ...
     * @param subIdIndex The index of the mSlotId column in the cursor
    ...
    public static class PhoneUtilsPreLMR1 extends PhoneUtils {
        ...
        @Override
        public String getDefaultSmsIccid(int subId) {
            return null;
        }
    ...
    public static class PhoneUtilsLMR1 extends PhoneUtils implements LMr1 {
        ...
        public int getDefaultSmsSubscriptionId() {
            ...
            if (systemDefaultSubId < 0) {
                // Always use -1 for any negative mSlotId from system
                return ParticipantData.DEFAULT_SELF_SUB_ID;
            }
        ...
        public String getDefaultSmsIccid(int subId) {
            ...
            String iccid = null;
            subInfo = ((SubscriptionManager) mContext.getSystemService(Context.TELEPHONY_SUBSCRIPTION_SERVICE)).getActiveSubscriptionInfo(subId);
            List<PhoneAccountHandle> subscriptionAccountHandles = PhoneAccountUtils.getSubscriptionPhoneAccounts(mContext);
            if (subInfo != null && subscriptionAccountHandles != null && !subscriptionAccountHandles.isEmpty()) {
                final int accIndex = subscriptionAccountHandles.size() > 1 ? subInfo.getSimSlotIndex() : 0;
                iccid = stripTrailingFs(subscriptionAccountHandles.get(accIndex).getId());
            }
            return iccid;
        }
        ...
        public boolean getHasPreferredSmsSim() {
            ...
            if(isBL01Device()&& (subId == DEFAULT_SIM_SETTING_ALWAYS_ASK_BL01)){
                return true;
            }else {
...
//        final String systemCountry = getLocaleCountry();
        //Bkav QuangNDb de mac dinh la viet nam de format cac so dien thoai la cung 1 kieu, do bi doi kieu format khi doi ngon ngu
        final String systemCountry = "VN";
