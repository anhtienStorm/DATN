...
public class UiUtils {
    ...
    // Them thong soo interpolator rieng cho ben minh
    public static final Interpolator BKAV_INTERPOLATOR = new CubicBezierInterpolator(
            0.26f, 0.9f, 0.17f, 0.97f);
     ...
     * @param onFinishRunnable an optional runnable called at the end of the animation
     */
    public static void revealOrHideViewWithAnimationBtalk(final View view, final int desiredVisibility,
                                                     @Nullable final Runnable onFinishRunnable) {
        final boolean needAnimation = view.getVisibility() != desiredVisibility;
        if (needAnimation) {
            final float fromScale = desiredVisibility == View.VISIBLE ? 0F : 1F;
            ...
            final ScaleAnimation showHideAnimation =
                    new ScaleAnimation(fromScale, toScale, fromScale, toScale,
                            ScaleAnimation.RELATIVE_TO_SELF, 0.5f,
                            ScaleAnimation.RELATIVE_TO_SELF, 0.5f);
            showHideAnimation.setDuration(MEDIAPICKER_TRANSITION_DURATION);
            showHideAnimation.setInterpolator(BKAV_INTERPOLATOR);
            showHideAnimation.setAnimationListener(new AnimationListener() {
                @Override
                public void onAnimationStart(final Animation animation) {
                }
    ...
    public static void setStatusBarColor(final Activity activity, final int color) {
        if (OsUtil.isAtLeastL()) {
            ...
            // Bkav TrungTH Test: sua lai mau nen statusbar => chuyen ve 0%
            final int blendedRed = Color.red(color)/*(int) Math.floor(0.95 * Color.red(color)*/;
            final int blendedGreen = /*(int) Math.floor(0.95 * Color.green(color))*/Color.green(color);
            final int blendedBlue = /*(int) Math.floor(0.95 * Color.blue(color))*/Color.blue(color);
