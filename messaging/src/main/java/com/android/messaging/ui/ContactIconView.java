...
public class ContactIconView extends AsyncImageView {
    ...
    protected boolean mDisableClickHandler;
    ...
    public ContactIconView(final Context context, final AttributeSet attrs) {
        ...
        final int iconSizeId = a.getInt(R.styleable.ContactIconView_iconSizeBtalk, 0);
        switch (iconSizeId) {
    ...
    public void setImageResourceUri(final Uri uri, final long contactId,
        ...
        if (uri == null) {
            ...
            if (AvatarUriUtil.TYPE_GROUP_URI.equals(avatarType)) {
                // Bkav QuangNDb tach code doan set group avater de override o class con
                setUpAvatarGroup(uri);
            } else {
                // Bkav QuangNDb tach code doan set personal avater de override o class con
                setUpAvatarPersonal(uri);
            }
        }
            ...
            if (!mDisableClickHandler) {
                setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(final View view) {
                        if (PhoneUtil.isPhoneNumber(mNormalizedDestination)) {
                            ContactUtil.showContactHaveUri(view, ContactInfoHelper.getContactInfoLookupUri(mNormalizedDestination));
                        }else {
    ...
    /**
     * ------------------------------------------------------------BKAV-----------------------------------------------------
     * Bkav QuangNDb: tach code doan set up avatar group thanh ham de override lai o lop con
     */
    protected void setUpAvatarGroup(final Uri uri) {
        setImageResourceId(new AvatarGroupRequestDescriptor(uri, mIconSize, mIconSize));
    }

    /**
     * Bkav QuangNDb: tach code doan set up avatar personal thanh ham de override lai o lop con
     */
    protected void setUpAvatarPersonal(final Uri uri){
        setImageResourceId(new AvatarRequestDescriptor(uri, mIconSize, mIconSize));
    }

    /**
     * Bkav QuangNDb: setUri cho imageview them conversationId de bin giong nhu ben contact
     */
    public void setImageResourceUri(Uri uri, long contactId, String contactLookupKey, String normalizedDestination,
                                    String conversationId, String displayName){
        //QuangNdb khong lam gi vi them moi o class con
    }
}
