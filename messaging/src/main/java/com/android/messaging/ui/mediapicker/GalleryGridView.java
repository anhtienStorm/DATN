...
public class GalleryGridView extends MediaPickerGridView implements
    ...
    public void onItemClicked(final View view, final GalleryGridItemData data,
        ...
        if (data.isDocumentPickerItem()) {
            ...
            // Bkav HienDTk: fix loi Tin nhắn - BOS 8.7 - Lỗi: Hiển thị thông báo "Ảnh bị lỗi" khi chọn ảnh từ bộ sưu tập => BOS-2651 - Start
            // Check th anh loi thi bao cho nguoi dung
            // Bkav HienDTk: khong check theo truong hop wight va height < 0 nua ma check bang bitmap
//            if (data.isImgError()) {
//                Toast.makeText(mContext, R.string.notify_image_error, Toast.LENGTH_SHORT).show();
//                return;
//            }
            try {
                Bitmap bitmap = MediaStore.Images.Media.getBitmap(mContext.getContentResolver(), data.getImageUri());
                // Bkav HienDTk: truong hop bitmap == null la anh bi loi
                if(bitmap == null){
                    Toast.makeText(mContext, R.string.notify_image_error, Toast.LENGTH_SHORT).show();
                    return;
                }
            ...
            // Bkav HienDTk: fix loi Tin nhắn - BOS 8.7 - Lỗi: Hiển thị thông báo "Ảnh bị lỗi" khi chọn ảnh từ bộ sưu tập => BOS-2651 - End
    ...
    public void onCreateOptionsMenu(final MenuInflater inflater, final Menu menu) {
        // Bkav QuangNDb tach code doan get id menu res
        inflater.inflate(getIdMenuRes(), menu);
    ...
    public boolean onOptionsItemSelected(final MenuItem item) {
        ...
        if (i == R.id.action_multiselect) {
            Assert.isTrue(canToggleMultiSelect());
            toggleMultiSelect();
            return true;
        } else if (i == R.id.action_confirm_multiselect) {
            Assert.isTrue(!canToggleMultiSelect());
    ...
    /**
     * --------------------------------------------------------------BKAV---------------------------------------
     * Bkav QUangNDB tach code doan get id menu
     */
    protected int getIdMenuRes() {
        return R.menu.gallery_picker_menu;
    }
}
