...
public class BugleDatabaseOperations {
     ...
     * @param refSubId The mSlotId used to normalize phone numbers in the recipients
     */
     ...
     * @param refSubId The reference mSlotId for canonicalize phone numbers
...
//            String defaultIccid = PhoneUtils.getDefault().getDefaultSmsIccid(defaultSubId);
    ...
    private static void updateConversationNameAndAvatarInTransaction(
        ...
        values.put(ConversationColumns.NAME_UNSIGN,
                convertToNotLatinCode(getDefaultConversationName(participants)));
        ...
        //Bkav QuangNDb update lai fts table
        dbWrapper.rebuildFtsConversationTable();
    ...
    private static String getParticipantId(final DatabaseWrapper dbWrapper,
        ...
        try {
            if (subId != ParticipantData.OTHER_THAN_SELF_SUB_ID) {
                ...
                cursor = dbWrapper.query(DatabaseHelper.PARTICIPANTS_TABLE,
                        new String[]{ParticipantColumns._ID},ParticipantColumns.SUB_ID + "=? ",
                        new String[]{Integer.toString(subId)}, null, null, null);
            } else {
                // Look for existing participant with this normalized phone number and no mSlotId.
                cursor = dbWrapper.query(DatabaseHelper.PARTICIPANTS_TABLE,
    ...
    /**Bkav QuangNDb lay des tu conversation id*/
    @DoesNotRunOnMainThread
    public static String getOtherParticipantDestinationFromConversation(
            final DatabaseWrapper db, final String conversationId) {
        Assert.isNotMainThread();
        Cursor cursor = null;
        try {
            cursor = db.query(DatabaseHelper.CONVERSATIONS_TABLE,
                    new String[] { ConversationColumns.OTHER_PARTICIPANT_NORMALIZED_DESTINATION },
                    ConversationColumns._ID + "=?",
                    new String[] { conversationId }, null, null, null);
            Assert.inRange(cursor.getCount(), 0, 1);
            if (cursor.moveToFirst()) {
...
//    =======================================================================Bkav================================================
    /**Bkav QuangNDb chuyen chu tu co dau sang thanh khong dau*/
    public static String convertToNotLatinCode(String latinCode) {
