...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String FTS_CONVERSATIONS_TABLE = "fts_conversations";
    ...
    public static final String CALL_RECORD_TABLE = "record_call_table";//Bkav QuangNDb bang record call
    public static final String QUICK_RESPONSE_TABLE = "quick_response";//Bkav QuangNDb bang quick response
    ...
    public static class ConversationColumns implements BaseColumns {
        ...
        /* Display name unsign quangndb bo xung de search khong dau */
        public static final String NAME_UNSIGN = "name_unsign";
    ...
    private static final String CREATE_CONVERSATIONS_TABLE_SQL =
            "CREATE TABLE " + CONVERSATIONS_TABLE + "("
                    ...
                    + ConversationColumns.NAME_UNSIGN + " TEXT"
                    + ");";

    //Bkav QuangNDb create conversation search table sql(full text search)
    private static final String CREATE_FTS_CONVERSATIONS_TABLE_SQL = "CREATE VIRTUAL TABLE " + FTS_CONVERSATIONS_TABLE
            + " USING fts4 (content='"+CONVERSATIONS_TABLE+"', "+ ConversationColumns.NAME_UNSIGN+");";
    ...
    public static class MessageColumns implements BaseColumns {
        ...
        /* QuangNDb them cot incoming_slot_id de luu thong tin message den la den sim 1 hay sim 2 */
        public static final String INCOMING_SLOT_ID = "incoming_slot_id";
        /* QuangNDb them cot iccid de biet tin nhan duoc thuc hien tu profile nao*/
    ...
    private static final String CREATE_MESSAGES_TABLE_SQL =
            "CREATE TABLE " + MESSAGES_TABLE + " ("
                    ...
                    + MessageColumns.INCOMING_SLOT_ID + " INT DEFAULT(-1), "
                    + MessageColumns.ICCID + " TEXT, " //bkav quangndb them
    ...
    public static class PartColumns implements BaseColumns {
        ...
        /*
         * Bkav QuangNDb them 1 cot text search de search co dau
         */
        public static final String TEXT_SEARCH = "text_search";

        //HienDTk: hanh dong goi contacts
        public static final String ACTION_CALL_CONTACTS = "action_call_contacts";
    }
    ...
    private static final String CREATE_PARTS_TABLE_SQL =
            "CREATE TABLE " + PARTS_TABLE + "("
                    ...
                    // Bkav QuangNDb them cot text search
                    + PartColumns.TEXT_SEARCH + " TEXT,"
    ...
    //Bkav QuangNDb bang luu thong tin file ghi am tam khi chua duoc cap quyen
    public static class CallRecordColumns implements BaseColumns {
    ...
    //Bkav QuangNDb bang luu thong tin quick response
    public static class QuickResponseColumns implements BaseColumns {
        public static final String RESPONSE = "response";
        public static final String IS_DEFAULT = "is_default";
    }

    //Bkav QuangNDb cau sql create record call table
    private static final String CREATE_CALL_RECORD_TABLE_SQL =
            "CREATE TABLE " + CALL_RECORD_TABLE + "("
                    + CallRecordColumns._ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "
                    + CallRecordColumns.DATE + " LONG, "
                    + CallRecordColumns.FROM + " TEXT, "
                    + CallRecordColumns.TO + " TEXT, "
                    + CallRecordColumns.SUB_ID + " INTEGER, "
                    + CallRecordColumns.TYPE + " INTEGER, "
                    + CallRecordColumns.FILE_PATH + " TEXT "
                    + ")";

    //Bkav QuangNDb cau sql create quick response table
    private static final String CREATE_QUICK_RESPONSE_TABLE_SQL =
            "CREATE TABLE " + QUICK_RESPONSE_TABLE + "("
                    + QuickResponseColumns._ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "
                    + QuickResponseColumns.RESPONSE + " TEXT, "
                    + QuickResponseColumns.IS_DEFAULT + " BOOL "
                    + ")";
    ...
    static final String DRAFT_PARTS_VIEW_SQL = "CREATE VIEW " +
            ...
            // Bkav QuangNDb them cot text search vao
            + PARTS_TABLE + '.' + PartColumns.TEXT_SEARCH
            + " as " + PartColumns.TEXT_SEARCH + ", "
    ...
    private static final String[] CREATE_TABLE_SQLS = new String[] {
        ...
        CREATE_CALL_RECORD_TABLE_SQL,//Bkav QuangNDb them sql create table call record
        CREATE_QUICK_RESPONSE_TABLE_SQL,//Bkav QuangNDb them sql create table quick response
    };

    // List of all our SQL fts tables
    private static final String[] CREATE_FTS_TABLE_SQLS = new String[] {
            CREATE_FTS_CONVERSATIONS_TABLE_SQL,
    };
    ...
    private static void createDatabase(final SQLiteDatabase db) {
        ...
        //Bkav QuangNDb create bang ao
        for (final String sql : CREATE_FTS_TABLE_SQLS) {
            db.execSQL(sql);
        }
