...
public class AvatarRequest extends UriImageRequest<AvatarRequestDescriptor> {
    ...
    // Bkav HienDTk: tao bien Avatar de lay LookupKey de custom mau cho avatar =>BOS-1472
    private  AvatarRequestDescriptor descriptorUri ;
    ...
    private Bitmap renderDefaultAvatar(final int width, final int height) {
        ...
        if (sDefaultPersonBitmap == null) {
            final BitmapDrawable defaultPerson = (BitmapDrawable) mContext.getResources()
                    .getDrawable(R.drawable.dafault_avatar);
    ...
    private int pickColor() {
        // Bkav HienDTk: Lỗi: Không mở được giao diện chi tiết liên hệ khi chạm vào liên hệ trong cài đặt Tin nhắn => BOS-1472 - Start
//        int colorRandom = new Random().nextInt(15 - 1) + 1;
//        String identifier = String.valueOf(colorRandom);
        String id = descriptorUri.uri.toString();
        // Bkav HienDTk: cat chuoi theo ky tư "="
        String[] splits = id.split("=");
        // Bkav HienDTk: lookupkey se la phan tu thu 3
        String identifier = splits[2];
        // Bkav HienDTk: Lỗi: Không mở được giao diện chi tiết liên hệ khi chạm vào liên hệ trong cài đặt Tin nhắn => BOS-1472 - End
