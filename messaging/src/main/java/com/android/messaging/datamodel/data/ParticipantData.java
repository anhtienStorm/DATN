...
public class ParticipantData implements Parcelable {
    ...
    public ContentValues toContentValues() {
        ...
        if (!isUnknownSender()) {
            // Bkav HuyNQN fix loi voi dau so viettel 032, 034, 035 bi mat so 0 khi format tai getFromRawPhoneBySystemLocale
            if(mDisplayDestination.length() == 9 && !mDisplayDestination.substring(0,1).equals("0")){
                String formatNumber = mNormalizedDestination.replaceFirst("(\\d{3})(\\d{4})(\\d+)", "$1 $2 $3");
                values.put(DatabaseHelper.ParticipantColumns.DISPLAY_DESTINATION, formatNumber);
            }else {
