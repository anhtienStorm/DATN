...
public class NoConfirmationSmsSendService extends IntentService {
    ...
    // Anhdts action bo qua tin nhan
    public static final String ACTION_ABORT = "bkav.android.btalk.action_abort";
    ...
    protected void onHandleIntent(final Intent intent) {
        ...
        if (ACTION_ABORT.equals(action)) {
            MarkAsReadAction.markAsRead(String.valueOf(intent.getStringExtra(UIIntents.UI_INTENT_EXTRA_CONVERSATION_ID)));
            UpdateMessageNotificationAction.updateMessageNotification();
            return;
        } else if (!TelephonyManager.ACTION_RESPOND_VIA_MESSAGE.equals(action)) {
            if (LogUtil.isLoggable(TAG, LogUtil.VERBOSE)) {
...
//            startActivity(new Intent(this, ConversationListActivity.class));
            // Bkav QuangNDb thay cach doi start activity de khong bi goi lai ConversationListActivity cu
            ...
            if (TextUtils.isEmpty(conversationId)) {
                ...
                //Bkav QuangNDb danh dau da doc cho cuoc hoi thoai khi tra loi nhanh
