...
public class ConversationMessageData {
    ...
    protected List<MessagePartData> mParts;
    private long mSentTimestamp;
    protected long mReceivedTimestamp;
    ...
    private int mIncomingSlotId = -1;
    private String mIccid;//Bkav QuangNDb them bien iccid

    // Bkav TienNAb: Them bien searchString de lay cac ky tu nhap vao khi search
    ...
    // Bkav TienNAb: ham lay gia tri cua bien searchString
    ...
    public void bind(final Cursor cursor) {
        ...
        mIncomingSlotId = cursor.getInt(INDEX_INCOMING_SLOT_ID);
        mIccid = cursor.getString(INDEX_ICCID);
    ...
    /**
     * QuangNDb bin cho viec seach tin nhan ngoai conversation list
     */
    public void binSearch(final Cursor cursor) {
        bind(cursor);
        // Bkav QuangNDb them ham lay du lieu conversation message item data
        mConversationListItemData = getConversationData(mConversationId);
    }

    /**
     * QuangNDb get incoming slot id cho tin nhan
     */
    public int getIncomingSlotId() {
        return mIncomingSlotId;
    }
    ...
    private boolean canClusterWithMessage(final Cursor cursor) {
        ...
        if (getConditionCanNotCluster(mReceivedTimestamp, otherReceivedTimestamp)) {
            return false;
        }
    ...
    public static final String getConversationMessagesQuerySql() {
        // Bkav TienNAb: set bien searchString bang null khi khong tim kiem
        mSearchString = null;
    ...
    private static final String CONVERSATION_MESSAGES_QUERY_PROJECTION_SQL =
            ...
            + " as " + ConversationMessageViewColumns.SENDER_CONTACT_LOOKUP_KEY + ", "
                    // QuangNDb them doan nay
                    + DatabaseHelper.MESSAGES_TABLE + '.' + MessageColumns.INCOMING_SLOT_ID
                    + " as " + ConversationMessageViewColumns.INCOMING_SLOT_ID + ", "
                    + DatabaseHelper.MESSAGES_TABLE + '.' + MessageColumns.ICCID
                    + " as " + ConversationMessageViewColumns.ICCID + " ";
    ...
    private static final String CONVERSATION_MESSAGES_QUERY_FROM_WHERE_SQL =
            ...
            // Bkav TienNAb: noi them bang conversations de query
            + " LEFT JOIN " + DatabaseHelper.CONVERSATIONS_TABLE
            + " ON (" + DatabaseHelper.MESSAGES_TABLE + '.' + MessageColumns.CONVERSATION_ID
            + "=" + DatabaseHelper.CONVERSATIONS_TABLE + '.' + DatabaseHelper.ConversationColumns._ID + ")"
    ...
    interface ConversationMessageViewColumns extends BaseColumns {
        ...
        static final String SENDER_NORMALIZED_DESTINATION = ParticipantColumns.NORMALIZED_DESTINATION;
        ...
        // QuangNDb them cot incomingSlotId
        static final String INCOMING_SLOT_ID = MessageColumns.INCOMING_SLOT_ID;
        static final String ICCID = MessageColumns.ICCID;
    }
    ...
    protected static final int INDEX_RECEIVED_TIMESTAMP            = sIndexIncrementer++;
    ...
    // QuangNDB them
    private static final int INDEX_INCOMING_SLOT_ID              = sIndexIncrementer++;
    private static final int INDEX_ICCID              = sIndexIncrementer++;
    ...
    private static String[] sProjection = {
        ...
        ConversationMessageViewColumns.ICCID,
    };
    ...
    /**
     * -----------------------------------------------------------------Bkav QuangNDb---------------------------------------------------
     * Them method kiem tra 2 message co khac ngay nhau khong, se override lai o lop con
     */
    ...
    /**
     * Them method kiem tra co phai la message cuoi cung khong
     */
    ...
    /**
     * Bkav QUangNDb ham check dieu kien khong phai 2 tin nhan cluster(source goc timestamp > 1' thi k phai
     */
    protected boolean getConditionCanNotCluster(long mReceivedTimestamp, long otherReceivedTimestamp) {
        ...
        return timestampDeltaMillis > DateUtils.MINUTE_IN_MILLIS;
    }

    // Bkav QuangNDb them ham query khi search message trong 1 conversation
    public static final String getConversationMessagesSearchQuerySql(String searchString) {
        // Bkav TienNAb: set gia tri cho bien queryString khi search
        mSearchString = searchString;
        return CONVERSATION_MESSAGES_QUERY_SQL
                + " AND "
                // Inject the conversation id
                + DatabaseHelper.MESSAGES_TABLE + "." + MessageColumns.CONVERSATION_ID
                + "=? and "+DatabaseHelper.MESSAGES_TABLE+"."+MessageColumns._ID+ " IN (Select "+ DatabaseHelper.PARTS_TABLE+"."+PartColumns.MESSAGE_ID
                +" From "+DatabaseHelper.PARTS_TABLE+" where UPPER("+DatabaseHelper.PARTS_TABLE+"."+PartColumns.TEXT_SEARCH
                +") LIKE UPPER('%"+searchString+"%'))"
                +")"
                + CONVERSATION_MESSAGES_QUERY_SQL_GROUP_BY;
    }

    // Bkav QuangNDb them ham query khi search message trong toan bo conversation
    /**Bkav QuangNDb Them dong lech loc conversation null trong where (sort timestamp =0) de khong bi crash khi co conversation null*/
    public static final String getMessagesSearchQuerySql(String searchString) {
        return CONVERSATION_MESSAGES_QUERY_SQL
                + " AND ("
                + DatabaseHelper.MESSAGES_TABLE+"."+MessageColumns._ID+ " IN (Select "+ DatabaseHelper.PARTS_TABLE+"."+PartColumns.MESSAGE_ID
                +" From "+DatabaseHelper.PARTS_TABLE+" where "+DatabaseHelper.PARTS_TABLE+"."+PartColumns.TEXT_SEARCH
                +" LIKE'%"+searchString+"%')"
                // Bkav TienNAb: them dieu kien se search theo ten da luu trong danh ba
                +" OR " + DatabaseHelper.MESSAGES_TABLE+"."+MessageColumns._ID+ " IN (Select "+ DatabaseHelper.MESSAGES_TABLE+"."+MessageColumns._ID
                +" From "+DatabaseHelper.MESSAGES_TABLE +" LEFT JOIN "+DatabaseHelper.CONVERSATIONS_TABLE+" ON ("+ DatabaseHelper.MESSAGES_TABLE + '.' + MessageColumns.CONVERSATION_ID
                + "=" + DatabaseHelper.CONVERSATIONS_TABLE + '.' + DatabaseHelper.ConversationColumns._ID + ")"
                +" where "+DatabaseHelper.CONVERSATIONS_TABLE+"."+ DatabaseHelper.ConversationColumns.NAME_UNSIGN
                // Bkav HaiKH - Fix bug BOS-3434- Start
                // Thêm điều kiện để search tin nhắn có dấu cách ở sđt
                +" LIKE'%"+searchString+"%' OR REPLACE("+DatabaseHelper.CONVERSATIONS_TABLE+"."+ DatabaseHelper.ConversationColumns.NAME_UNSIGN+",' ','')"
                +" LIKE'%"+searchString+"%'))"
                // Bkav HaiKH - Fix bug BOS-3434-END
                +" AND "+DatabaseHelper.MESSAGES_TABLE+"."+MessageColumns.CONVERSATION_ID+" IN (Select "+ DatabaseHelper.CONVERSATIONS_TABLE+"."+ DatabaseHelper.ConversationColumns._ID
                +" From "+DatabaseHelper.CONVERSATIONS_TABLE+" where "+ DatabaseHelper.ConversationColumns.SORT_TIMESTAMP+" > 0)"
                // Bkav TienNAb: khi khong co ket qua khi search bang noi dung tin nhan thi group by theo id conversation
                +") GROUP BY"
                + " CASE WHEN ( "+ DatabaseHelper.MESSAGES_TABLE+"."+MessageColumns._ID+ " NOT IN (Select "+ DatabaseHelper.PARTS_TABLE+"."+PartColumns.MESSAGE_ID
                +" From "+DatabaseHelper.PARTS_TABLE+" where "+DatabaseHelper.PARTS_TABLE+"."+PartColumns.TEXT_SEARCH
                +" LIKE'%"+searchString+"%'))"
                +" THEN "+ DatabaseHelper.CONVERSATIONS_TABLE+"."+ DatabaseHelper.ConversationColumns._ID
                +" ELSE "+ DatabaseHelper.MESSAGES_TABLE+"."+MessageColumns._ID +" END"
                + " ORDER BY "
                + DatabaseHelper.MESSAGES_TABLE + '.' + MessageColumns.RECEIVED_TIMESTAMP + " DESC";
    }

    /**
     * Bkav QuangNDb lay du lieu conversation list item
     */
    public ConversationListItemData getConversationListItemData() {
        return mConversationListItemData;
    }

    /**
     * Bkav QuangNDb lay du lieu conversation listitem data
     */
    public ConversationListItemData getConversationData(String conversationId) {
        Cursor cursor = null;
        ConversationListItemData listItemData = new ConversationListItemData();
        try {
     ...
     * Anhdts get sql mms
     */
    public static final String getConversationMmsQuerySql() {
        return CONVERSATION_MESSAGES_QUERY_SQL
                + " AND "
                // Inject the conversation id
                + DatabaseHelper.MESSAGES_TABLE + "." + MessageColumns.CONVERSATION_ID + "=?1 AND "
                + DatabaseHelper.MESSAGES_TABLE + "." + MessageColumns.SMS_MESSAGE_URI + " like ?2)"
                + CONVERSATION_MESSAGES_QUERY_SQL_GROUP_BY;
    }

    /**
     * Anhdts get sql audio
     */
    public static final String getConversationAudioQuerySql() {
        return "SELECT * FROM (" + CONVERSATION_MESSAGES_QUERY_SQL
                + " AND "
                // Inject the conversation id
                + DatabaseHelper.MESSAGES_TABLE + "." + MessageColumns.CONVERSATION_ID + "=?1AND "
                + DatabaseHelper.MESSAGES_TABLE + "." + MessageColumns.SMS_MESSAGE_URI + " like ?2)"
                + CONVERSATION_MESSAGES_QUERY_SQL_GROUP_BY +") as temp_Table WHERE temp_table."
                + ConversationMessageViewColumns.PARTS_CONTENT_TYPES + " like ?3";
    }

    // Bkav TienNAb: get sql sms
    public static final String getConversationSmsQuerySql() {
        return CONVERSATION_MESSAGES_QUERY_SQL
                + " AND "
                // Inject the conversation id
                + DatabaseHelper.MESSAGES_TABLE + "." + MessageColumns.CONVERSATION_ID + "=?1AND "
