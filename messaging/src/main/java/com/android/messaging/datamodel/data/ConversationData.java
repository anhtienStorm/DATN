...
public class ConversationData extends BindableData {
    ...
    private class MessagesLoaderCallbacks implements LoaderManager.LoaderCallbacks<Cursor> {
        ...
        public Loader<Cursor> onCreateLoader(final int id, final Bundle args) {
            ...
            // Check if data still bound to the requesting ui element
            if (isBound(bindingId)) {
                // Bkav QuangNDb them de nhanh uri khi search hoac khong search
                // Anhdts them mode loc mms
                final Uri uri = mSearchString == null ? mTypeFilter == FILTER_MMS ?
                        MessagingContentProvider.buildConversationMMSUri(mConversationId) :
                        mTypeFilter == FILTER_AUDIO ? MessagingContentProvider.buildConversationAudioUri(mConversationId) :
                        // Bkav TienNAb: build uri khi chon chuc nang loc sms
                        mTypeFilter == FILTER_SMS ? MessagingContentProvider.buildConversationSmsUri(mConversationId):
                                MessagingContentProvider.buildConversationMessagesUri(mConversationId):
                        MessagingContentProvider.buildConversationMessagesSearchUri(mConversationId,mSearchString);
                loader = new BoundCursorLoader(bindingId, mContext, uri,
    ...
    // Bkav QuangNDb them ham restart loader khi search
     ...
     */
    public SubscriptionListEntry getSubscriptionEntryForSelfParticipant() {
        return getSubscriptionEntryForSelfParticipant(mSubscriptionListData, mSelfParticipantsData);
    }
    ...
    public static SubscriptionListEntry getSubscriptionEntryForSelfParticipant(
        ...
        int count = selfParticipantsData.getSelfParticipantsCountExcludingDefault(true);
        //Bkav QuangNDb them dieu kien de khi co 1 sim la eSim co nhieu profile thi cung tinh la multisim
        if (OsUtil.isAtLeastL_MR1() &&
                (selfParticipantsData.getSelfParticipantsCountExcludingDefault(true) > 1 || countSim()>1)) {
     ...
     * QuangNDb
     */
    public static SubscriptionListEntry getSubscriptionEntryForSelfParticipant(final SubscriptionListData subscriptionListData,
            final SelfParticipantsData selfParticipantsData) {
        // SIM indicators are shown in the UI only if:
        // 1. Framework has MSIM support AND
        // 2. The device has had multiple *active* subscriptions. AND
        ...
        if (OsUtil.isAtLeastL_MR1() &&
                (selfParticipantsData.getSelfParticipantsCountExcludingDefault(true) > 1 || countSim()>1)) {
            return subscriptionListData.getDefaultEntry();
        }
        return null;
    }
    ...
    public static final int FILTER_MMS = 1;
    public static final int FILTER_AUDIO = 2;
    // Bkav TienNAb: them filter sms
    public static final int FILTER_SMS = 3;

    /**
     * Anhdts loc tin nhan MMS, audio
     */
    public void filterMMSAudio(final LoaderManager loaderManager,
                               final BindingBase<ConversationData> binding, int typeFilter) {
        // Remember the binding id so that loader callbacks can check if data is still bound
        // to same ui component
        if (mTypeFilter == typeFilter) {
            return;
        }
    ...
    // Bkav TienNAb: loc tin nhan SMS
    public void filterSMS(final LoaderManager loaderManager,
                               final BindingBase<ConversationData> binding, int typeFilter) {
        // Remember the binding id so that loader callbacks can check if data is still bound
        // to same ui component
        if (mTypeFilter == typeFilter) {
    ...
    //Bkav QuangNDb them db de xu ly trong truong hop 1 sim co nhieu esim
    private static final String AUTHORITY = "bkav.android.esim.simmanager";
    private static final String SIM_TABLE = "sim_info";
    private static final String SIM_SLOT = "sim_slot";
    private static final String URL = "content://" + AUTHORITY + "/" + SIM_TABLE;
    private static final Uri CONTENT_URI = Uri.parse(URL);

    //Bkav QuangNDb check xem may dang co bao nhieu sim dang enable
    private static int countSim() {
        int count = 0;
        Cursor cursor = Factory.get().getApplicationContext().getContentResolver().query(CONTENT_URI,
                null, null, null, null);
        if (cursor != null && cursor.getCount() > 0 && cursor.moveToFirst()) {
            do {
                final String slot = cursor.getString(cursor.getColumnIndex(SIM_SLOT));
                if (!isSlotNotReady(Integer.parseInt(slot))) {
                    count++;
                }
    ...
    private static boolean isSlotNotReady(int slot) {
