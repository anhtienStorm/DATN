...
public abstract class MessageNotificationState extends NotificationState {
    ...
    static class ConversationLineInfo {
        ...
        // Incoming slot id QuangNDb them
        int mIncomingSlotId ;
        ...
        public ConversationLineInfo(final String conversationId,
            ...
            mIncomingSlotId = incomingSLotId;
        }

        public void setIncomingSlotId(int mIncomingSlotId) {
            this.mIncomingSlotId = mIncomingSlotId;
        }
    ...
    public static class MultiConversationNotificationState extends MessageNotificationState {
        ...
        public MultiConversationNotificationState(
            ...
            int messageCount = convList.mConvInfos.size();
            // QuangNDb neu la nhieu tin nhan thi set icon sim1, sim2 cho tin nhan cuoi cung
            mSlotId = -1;
            for (int i = 0; i < messageCount; i++) {
                ...
                BundledMessageNotificationState childState = new BundledMessageNotificationState(list, i);
                // QuangNDb them slot cho tung tin nhan trong nhieu cuoc hoi thoai
                childState.mSlotId = convInfo.mIncomingSlotId;
                mChildren.add(childState);
            }
...
//                if (mSlotId == 0) {
//                    return R.drawable.ic_sim_small_1;
//                } else if (mSlotId == 1) {
//                    return R.drawable.ic_sim_small_2;
//                }
//            }
            // Anhdts tren b1 thi bo ava nguoi va thay bang icon tin nhan
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP_MR1) {
                return R.drawable.ic_sim_small_0;
            } else {
                return R.drawable.bkav_icon_mms;
            }
        }
    ...
    public static class BundledMessageNotificationState extends MultiMessageNotificationState {
        ...
        public BundledMessageNotificationState(final ConversationInfoList convList, final int groupOrder) {
            super(convList);
                ...
                do {
                    ...
                    if (currConvInfo == null) {
                        ...
                        currConvInfo = new ConversationLineInfo(convId,
                                ...
                                convMessageData.getIncomingSlotId());
                        convLineInfos.put(convId, currConvInfo);
                    }
                    currConvInfo.setIncomingSlotId(convMessageData.getIncomingSlotId());
...
//            int slotId = convList.mConvInfos.get(0).mIncomingSlotId;
//            //QuangNDb
...
//                state.setSlotId(slotId);
                ...
                // QuangNDb them
                state.setSlotId(convInfo.mIncomingSlotId);
            }
        }
...
//                        }
    ...
    /**
     * ------------------------------------------------------BKAV--------------------------------
     * Bkav QuangNDb thay icon notify cua message
     * todo Phai thay truc tiep do cac class co goi class nay deu la static khong extend duoc
     */
    private int getIdResSMSNotifyIcon() {
//        return R.drawable.ic_sms_light;
        // QuangNDb neu la 2 sim thi check icon
//        if (PhoneUtils.getDefault().getActiveSubscriptionCount() > 1) {
//            if (mSlotId == 0) {
//                return R.drawable.ic_sim_small_1;
//            } else if (mSlotId == 1) {
...
//        }
        // Anhdts tren b1 thi bo ava nguoi va thay bang icon tin nhan
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP_MR1) {
            // Bkav HienDTk: thay icon messager tren thanh status bar

            // Bkav HaiKH - Fix bug BOS-2998- Start
            // Icon notification trên status bar
            return R.drawable.icon_message_notification;
            // Bkav HaiKH - Fix bug BOS-2998- End
        } else {
            return R.drawable.bkav_icon_mms;
        }
    }
