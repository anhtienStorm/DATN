...
public class ReceiveSmsMessageAction extends Action implements Parcelable {
    ...
    protected Object executeAction() {
        ...
        // QuangNDb get the sim slot id
        int slotID = messageValues.getAsInteger("slot");
        messageValues.remove("slot");
        //Bkav QuangNDb them iccid
        String iccid = messageValues.getAsString("iccid");
        messageValues.remove("iccid");
        ...
        if (!OsUtil.isSecondaryUser()) {
            ...
            try {
                ...
                message = MessageData.createReceivedSmsMessage(messageUri, conversationId,
                        participantId, selfId, text, subject, sent, received, seen, read, slotID, iccid);
        ...
        // QuangNDb fix de khi show notification hien sim1, sim2
        BugleNotifications.update(false/*silent*/, conversationId, BugleNotifications.UPDATE_ALL);
//        BugleNotifications.updateNotification(false/*silent*/, conversationId, BugleNotifications.UPDATE_ALL, slotID);
