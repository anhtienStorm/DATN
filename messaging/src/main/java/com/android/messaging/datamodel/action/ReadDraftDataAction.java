...
public class ReadDraftDataAction extends Action implements Parcelable {
    ...
    protected Object executeAction() {
        ...
        //Bkav QuangNDb Them truong hop neu chua co conversationId van load draf de khong bi loi khi share tin nhan
        if (conversation != null || "-1".equals(conversationId)) {
            if (incomingDraft == null) {
                message = BugleDatabaseOperations.readDraftMessageData(db, conversationId,
                        conversation != null?conversation.getSelfId():"-1");
            }
            if (message == null) {
                message = MessageData.createDraftMessage(conversationId, conversation != null?conversation.getSelfId():"-1",
                        incomingDraft);
                //Bkav QuangNDb xoa log
