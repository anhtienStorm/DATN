...
public class MessagingContentProvider extends ContentProvider {
    ...
    // Bkav TrungTH - Sua com => bkav de khong trung voi csdl goc (conflic khong cai ngoai duoc)
    @VisibleForTesting
    public static final String AUTHORITY =
            "bkav.android.messaging.datamodel.MessagingContentProvider";
    ...
    // Bkav HienDTk: Bang conversation_participants va tao uri tro den bang => BOS-2335 - Start
    private static final String PARTICIPANTS_CONVERSATIONS = "conversation_participants";
    public static final Uri PARTICIPANTS_CONVERSATIONS_URI = Uri.parse(CONTENT_AUTHORITY + PARTICIPANTS_CONVERSATIONS);
    // Bkav HienDTk: Bang conversation_participants va tao uri tro den bang => BOS-2335 - End

    // Bkav QuangNDb conversation query all
    private static final String CONVERSATIONS_QUERY_ALL = "conversations_all";
    public static final Uri CONVERSATIONS_URI_ALL = Uri.parse(CONTENT_AUTHORITY + CONVERSATIONS_QUERY_ALL);

    public static final Uri CONVERSATIONS_URI = Uri.parse(CONTENT_AUTHORITY + CONVERSATIONS_QUERY);
    public static final Uri PARTS_URI = Uri.parse(CONTENT_AUTHORITY + DatabaseHelper.PARTS_TABLE);
    ...
    //Bkav QuangNDb Call record query
    private static final String CALL_RECORD_QUERY = "call_record_table";
    public static final Uri CALL_RECORD_URI = Uri.parse(CONTENT_AUTHORITY + CALL_RECORD_QUERY);

    //Bkav QuangNDb quick response
    private static final String QUICK_RESPONSE_QUERY = "quick_response";
    public static final Uri QUICK_RESPONSE_URI = Uri.parse(CONTENT_AUTHORITY + QUICK_RESPONSE_QUERY);
    ...
    // Bkav QUangNDb them cac nhanh de query
    private static final int CONVERSATION_ALL_QUERY_CODE = 80;
    private static final int PARTS_QUERY_CODE = 90;
    private static final int CONVERSATION_MESSAGES_QUERY_SEARCH_CODE = 100;
    private static final int MESSAGES_QUERY_SEARCH_CODE = 110;
    private static final int MESSAGES_QUERY_UNREAD_CODE = 120;
    private static final int CALL_RECORD_CODE = 130;
    private static final int QUICK_RESPONSE_CODE = 140;//Bkav QuangNDb them code uri quick response
    private static final int CONVERSATION_QUERY_MMS = 150; // Anhdts query mms
    private static final int CONVERSATION_QUERY_AUDIO = 160; // Anhdts query audio

    // Bkav TienNAb: Query sms
    private static final int CONVERSATION_QUERY_SMS = 170;
    // Bkav HienDTk: query conversation_participants
    private static final int PARTICIPANTS_CONVERSATIONS_QUERY = 180;
    ...
    static {
        ...
        // Bkav QuangNDb them cac uri moi de query phu hop hon
        sURIMatcher.addURI(AUTHORITY, CONVERSATIONS_QUERY_ALL, CONVERSATION_ALL_QUERY_CODE);
        sURIMatcher.addURI(AUTHORITY, DatabaseHelper.PARTS_TABLE, PARTS_QUERY_CODE);
        sURIMatcher.addURI(AUTHORITY, MESSAGES_QUERY + "/conversation/*/*", CONVERSATION_MESSAGES_QUERY_SEARCH_CODE);
        sURIMatcher.addURI(AUTHORITY, MESSAGES_QUERY+ "/unread", MESSAGES_QUERY_UNREAD_CODE);

        // Anhdts them uri query MMS
        sURIMatcher.addURI(AUTHORITY, MESSAGES_QUERY + "/mms/" + CONVERSATIONS_QUERY + "/*", CONVERSATION_QUERY_MMS);
        sURIMatcher.addURI(AUTHORITY, MESSAGES_QUERY + "/audio/" + CONVERSATIONS_QUERY + "/*", CONVERSATION_QUERY_AUDIO);

        // Bkav TienNAb: them uri query SMS
        sURIMatcher.addURI(AUTHORITY, MESSAGES_QUERY + "/sms/conversation/*", CONVERSATION_QUERY_SMS);
        // Bkav HienDTk: them uri query bang participants_conversations => BOS-2335 - Start
        sURIMatcher.addURI(AUTHORITY, PARTICIPANTS_CONVERSATIONS, PARTICIPANTS_CONVERSATIONS_QUERY);
        // Bkav HienDTk: them uri query bang participants_conversations => BOS-2335 - End

        sURIMatcher.addURI(AUTHORITY, MESSAGES_QUERY + "/*", MESSAGES_QUERY_SEARCH_CODE);
        sURIMatcher.addURI(AUTHORITY, CALL_RECORD_QUERY, CALL_RECORD_CODE);
        sURIMatcher.addURI(AUTHORITY, QUICK_RESPONSE_QUERY, QUICK_RESPONSE_CODE);
    }
    ...
    /**
     * QuangNDb build uri khi search meaasge trong 1 conversation
     * Build a messages uri from the conversation id.
     */
    public static Uri buildConversationMessagesSearchUri(final String conversationId, final String searchString) {
        final Uri.Builder builder = CONVERSATION_MESSAGES_URI.buildUpon();
        builder.appendPath(conversationId);
    ...
    /**
     * QuangNDb build uri khi search message trong toan bo conversation
     * Build a messages uri from the conversation id.
     */
    public static Uri buildMessagesSearchUri(final String searchString) {
        final Uri.Builder builder = MESSAGES_URI.buildUpon();
        builder.appendPath(searchString);
        return builder.build();
    }
    ...
    public Cursor query(final Uri uri, final String[] projection, String selection,
        ...
        switch (match) {
            ...
            case DRAFT_IMAGES_QUERY_CODE:
                ...
                break;
            // Bkav QuangNDb them doan nay
            case CONVERSATION_ALL_QUERY_CODE:
                queryBuilder.setTables(DatabaseHelper.CONVERSATIONS_TABLE);
                // Hide empty conversations (ones with 0 sort_timestamp)
                queryBuilder.appendWhere(ConversationColumns.SORT_TIMESTAMP + " > 0 ");
                break;
            // BKav QUangNDb them code de query toan bo bang part message
            case PARTS_QUERY_CODE:
                queryBuilder.setTables(DatabaseHelper.PARTS_TABLE);
                break;
            // BKav QUangNDb them code de query doan search tin nhan trong 1 conversation
            case CONVERSATION_MESSAGES_QUERY_SEARCH_CODE:
                if (uri.getPathSegments().size() == 4 && TextUtils.equals(uri.getPathSegments().get(1), "conversation")) {
                    // Get the conversation id from the uri
                    final String conversationId = uri.getPathSegments().get(2);
                    final String searchString = uri.getPathSegments().get(3);
                    // We need to handle this query differently, instead of falling through to the
                    // generic query call at the bottom. For performance reasons, the conversation
                    // messages query is executed as a raw query. It is invalid to specify
                    // selection/sorting for this query.
                    if (selection == null && selectionArgs == null && sortOrder == null) {
                        return queryConversationMessagesSearch(conversationId,searchString, uri);
                    } else {
                        throw new IllegalArgumentException(
                                "Cannot set selection or sort order with this query");
                    }
                ...
                // Bkav QuanNDb them code de query doan search toan bo tin nhan
            case MESSAGES_QUERY_SEARCH_CODE:
                final String searchString = uri.getPathSegments().get(1);
                // We need to handle this query differently, instead of falling through to the
                // generic query call at the bottom. For performance reasons, the conversation
                // messages query is executed as a raw query. It is invalid to specify
                // selection/sorting for this query.
                if (selection == null && selectionArgs == null && sortOrder == null) {
                    return queryMessagesSearch(searchString, uri);
                } else {
                    throw new IllegalArgumentException(
                            "Cannot set selection or sort order with this query");
                }
                // BKav QUangNDb them code de query toan bo bang message
            case MESSAGES_QUERY_UNREAD_CODE:
                queryBuilder.setTables(DatabaseHelper.MESSAGES_TABLE);
                queryBuilder.appendWhere(DatabaseHelper.MessageColumns.READ + " = 0 ");
                break;
            case CALL_RECORD_CODE:
                queryBuilder.setTables(DatabaseHelper.CALL_RECORD_TABLE);
                break;
            case QUICK_RESPONSE_CODE:
                queryBuilder.setTables(DatabaseHelper.QUICK_RESPONSE_TABLE);
                break;
            // Anhdts query list mms
            case CONVERSATION_QUERY_MMS:
                if (uri.getPathSegments().size() == 4) {
                    // Get the conversation id from the uri
                    final String conversationId = uri.getPathSegments().get(3);
                    ...
                    // selection/sorting for this query.
                    if (selection == null && selectionArgs == null && sortOrder == null) {
                        return queryConversationMms(conversationId, uri);
                    } else {
                        throw new IllegalArgumentException(
                                "Cannot set selection or sort order with this query");
                    }
            ...
            case CONVERSATION_QUERY_AUDIO:
                if (uri.getPathSegments().size() == 4) {
                    ...
                    // selection/sorting for this query.
                    if (selection == null && selectionArgs == null && sortOrder == null) {
                        return queryConversationAudio(conversationId, uri);
                    } else {
                        throw new IllegalArgumentException(
                                "Cannot set selection or sort order with this query");
                    }
            ...
            // Bkav TienNAb: query list sms
            case CONVERSATION_QUERY_SMS:
                if (uri.getPathSegments().size() == 4 &&
                        TextUtils.equals(uri.getPathSegments().get(1), "sms")) {
                    // Get the conversation id from the uri
                    final String conversationId = uri.getPathSegments().get(3);
                    ...
                    // selection/sorting for this query.
                    if (selection == null && selectionArgs == null && sortOrder == null) {
                        return queryConversationSms(conversationId, uri);
                    } else {
                        throw new IllegalArgumentException(
                                "Cannot set selection or sort order with this query");
                    }
                ...
                // Bkav HienDTk: query bang conversation_participants
            case PARTICIPANTS_CONVERSATIONS_QUERY:
                queryBuilder.setTables(DatabaseHelper.CONVERSATION_PARTICIPANTS_TABLE);
                break;
            default: {
    ...
    public Uri insert(final Uri uri, final ContentValues values) {
        ...
        switch (sURIMatcher.match(uri)) {
            case CALL_RECORD_CODE:
                id = sqLiteDatabase.insert(DatabaseHelper.CALL_RECORD_TABLE, null, values);
                break;
            case QUICK_RESPONSE_CODE:
                id = sqLiteDatabase.insert(DatabaseHelper.QUICK_RESPONSE_TABLE, null, values);
                break;
            default:
    ...
    public int delete(final Uri uri, final String selection, final String[] selectionArgs) {
        ...
        switch (sURIMatcher.match(uri)) {
            case CALL_RECORD_CODE:
                rowDelete = sqLiteDatabase.delete(DatabaseHelper.CALL_RECORD_TABLE, selection, selectionArgs);
                break;
            case QUICK_RESPONSE_CODE:
                rowDelete = sqLiteDatabase.delete(DatabaseHelper.QUICK_RESPONSE_TABLE, selection, selectionArgs);
                break;
            default:
    ...
    public int update(final Uri uri, final ContentValues values, final String selection,
        ...
        switch (sURIMatcher.match(uri)) {
            ...
            // Thêm value để update bảng conversation.
            case CONVERSATION_ALL_QUERY_CODE:
                rowUpdate = sqLiteDatabase.update(DatabaseHelper.CONVERSATIONS_TABLE, values, selection,
                        selectionArgs);
                break;
            // Bkav HaiKH - Fix bug BOS-3732- End
            case QUICK_RESPONSE_CODE:
                rowUpdate = sqLiteDatabase.update(DatabaseHelper.QUICK_RESPONSE_TABLE, values, selection,
                        selectionArgs);
                break;
            default:
    ...
    //========================================================================BKAV===============================================
    /**
     * Bkav QuangNDb them phan query cho search message uri cua 1 conversation
     */
    private Cursor queryConversationMessagesSearch(final String conversationId, final String searchString, final Uri notifyUri) {
        final String[] queryArgs = { conversationId };
        final Cursor cursor = getDatabaseWrapper().rawQuery(
                ConversationMessageData.getConversationMessagesSearchQuerySql(searchString), queryArgs);
        cursor.setNotificationUri(getContext().getContentResolver(), notifyUri);
        return cursor;
    }

    /**
     * Bkav QuangNDb them phan query cho search message uri cua toan be message
     */
    private Cursor queryMessagesSearch(final String searchString, final Uri notifyUri) {
        final Cursor cursor = getDatabaseWrapper().rawQuery(
                ConversationMessageData.getMessagesSearchQuerySql(searchString), null);
        cursor.setNotificationUri(getContext().getContentResolver(), notifyUri);
        return cursor;
    }
     ...
     * Anhdts query list mms
     */
    public static Uri buildConversationMMSUri(final String conversationId) {
        final Uri.Builder builder = Uri.parse(CONTENT_AUTHORITY +
                MESSAGES_QUERY + "/mms/" + CONVERSATIONS_QUERY).buildUpon();
        builder.appendPath(conversationId);
        return builder.build();
    }

    /**
     * Anhdts query list ghi am
     */
    public static Uri buildConversationAudioUri(final String conversationId) {
        final Uri.Builder builder = Uri.parse(CONTENT_AUTHORITY +
                MESSAGES_QUERY + "/audio/" + CONVERSATIONS_QUERY).buildUpon();
        builder.appendPath(conversationId);
        return builder.build();
    }

    // Bkav TienNAb: query list sms
    public static Uri buildConversationSmsUri(final String conversationId) {
        final Uri.Builder builder = Uri.parse(CONTENT_AUTHORITY +
                MESSAGES_QUERY + "/sms/conversation").buildUpon();
        builder.appendPath(conversationId);
        return builder.build();
    }
     ...
     * Anhdts query mms
     */
    private Cursor queryConversationMms(final String conversationId, final Uri notifyUri) {
        final String[] queryArgs = { conversationId, "%mms%"};
        final Cursor cursor = getDatabaseWrapper().rawQuery(
                ConversationMessageData.getConversationMmsQuerySql(), queryArgs);
        cursor.setNotificationUri(getContext().getContentResolver(), notifyUri);
        return cursor;
    }

    // Bkav TienNAb: query sms
    private Cursor queryConversationSms(final String conversationId, final Uri notifyUri) {
        final String[] queryArgs = { conversationId, "%sms%"};
        final Cursor cursor = getDatabaseWrapper().rawQuery(
                ConversationMessageData.getConversationSmsQuerySql(), queryArgs);
        cursor.setNotificationUri(getContext().getContentResolver(), notifyUri);
        return cursor;
    }
     ...
     * Anhdts query audio
     */
    private Cursor queryConversationAudio(final String conversationId, final Uri notifyUri) {
        final String[] queryArgs = { conversationId , "%mms%", "%audio%"};
        final Cursor cursor = getDatabaseWrapper().rawQuery(ConversationMessageData.getConversationAudioQuerySql(), queryArgs);
        cursor.setNotificationUri(getContext().getContentResolver(), notifyUri);
        return cursor;
    }
}