...
public class ContactListItemData {
    ...
    /**
     * -----------------------------------------------------------BKAV-----------------------------------------------------------------
     * Bkav quangndb: ham lay 1 ContactListItemData dua tren 1 so dien thoai truyen vao
     */
    public static ContactListItemData getExistingContactListItemData(Context context, String phoneNum, String alphabetHeader) {
        ContactListItemData data = null;
        Cursor cursor = null;
        final Uri uri = ContactsContract.CommonDataKinds.Phone.CONTENT_URI.buildUpon().appendQueryParameter(
                ...
                .build();
        String projection[] = ContactUtil.PhoneQuery.PROJECTION;
        String selection = ContactsContract.CommonDataKinds.Phone.NUMBER + " = '" + phoneNum + "'";
        try {
            cursor = context.getContentResolver().query(uri, projection, selection, null, ContactUtil.PhoneQuery.SORT_KEY);
            if (cursor != null && cursor.getCount() > 0) {
                cursor.moveToFirst();
                data = new ContactListItemData();
                data.bind(cursor, alphabetHeader);
            }
        } finally {
            if (cursor != null) {
                cursor.close();
