...
public class SimSelectorAvatarRequest extends AvatarRequest {
    ...
    private ImageResource renderSimAvatarInternal(final String identifier, final int width,
        ...
        if (sRegularSimIcon == null) {
            final BitmapDrawable regularSim = getRegularSim(width);
...
//        ContextCompat.getDrawable(Factory.get().getApplicationContext(),).
        ...
        if (!TextUtils.isEmpty(identifier)) {
            ...
            final float letterToTileRatio =
                    resources.getFraction(getSimTitleRatio(width), 1, 1);
    ...
    /**Bkav QuangNDb get background color*/
    protected int getSimBackgroundColor(boolean selected, int subColor) {
        return selected ? subColor : Color.WHITE;
    }
    ...
    //==============================================================BKAV======================================
    /**
    * Bkav QuangNDb ham get dimens value ratio(% co chu trong canvas)
     * @param width
     */
    protected int getSimTitleRatio(int width) {
        return R.dimen.sim_identifier_to_tile_ratio;
    }

    /**Bkav QuangNDb get bitmap sim
     * @param width*/
