    ...
    private static void refreshSelfParticipantList() {
        ...
        try {
            ...
            for (final Integer subId : activeSubscriptionIdToRecordMap.keySet()) {
                ...
                // Anhdts khong biet vi ly do gi do ma thong tin nay bi null, nghi do sim loi
...
//        String defaultIccid = PhoneUtils.getDefault().getDefaultSmsIccid(defaultSubId);
    ...
    /**Bkav QuangNDb runnable thuc hien viec thong bao change database local khi thay doi 1 contact*/
    static class RefreshOnlyChangeRunnable implements Runnable{

        private String mContactId = "-1";
        public RefreshOnlyChangeRunnable(String contactId) {
        ...
        public void run() {
            final boolean oldScheduled = sFullRefreshScheduled.getAndSet(false);
            Assert.isTrue(oldScheduled);
            refreshParticipants(mContactId);
        }
    }
    /**
     * Refresh participants only if needed, i.e., application start or contact changed.
     */
    public static void refreshParticipantsIfNeeded(final String contactId) {
        if (ParticipantRefresh.getNeedFullRefresh() &&
                sFullRefreshScheduled.compareAndSet(false, true)) {
            if (LogUtil.isLoggable(TAG, LogUtil.VERBOSE)) {
                LogUtil.v(TAG, "Started full participant refresh");
            }
            final Runnable RefreshOnlyChangeRunnable = new RefreshOnlyChangeRunnable(contactId);
            SafeAsyncTask.executeOnThreadPool(RefreshOnlyChangeRunnable);
        } else if (LogUtil.isLoggable(TAG, LogUtil.VERBOSE)) {
            LogUtil.v(TAG, "Skipped full participant refresh");
        }
    }
    ...
    /**
     * Refresh participants in Bugle with contact id.
     * QuangNDb custom
     */
    @VisibleForTesting
    static void refreshParticipants(String contactId) {
        ...
        selection = ParticipantColumns._ID + "=?";
