...
public class UpdateConversationOptionsAction extends Action
    ...
    protected Object executeAction() {
        ...
        // update âm báo tin nhắn khi thay đổi cài đặt trong tin nhắn.
        updateRingtone(getIdContact(conversationId));
        // Bkav HaiKH - Fix bug BOS-3732- End
        return null;
    }

    // Bkav HaiKH - Fix bug BOS-3732- Start
    // Hàm lấy id contact từ bảng conversation
    private String getIdContact(String id){
        String[] args = { id };
        Cursor cursor = null;
        String idContact = null;
        String[] projectionMessage = new String[] {
                ConversationColumns._ID,
                ConversationColumns.PARTICIPANT_CONTACT_ID
        };
        try {
            cursor = getContext().getContentResolver().query(
                    MessagingContentProvider.CONVERSATIONS_URI_ALL, projectionMessage,
                    ConversationColumns._ID + " = ?", args, null);
            if (cursor != null && cursor.getCount() > 0) {
                cursor.moveToFirst();
                idContact = cursor.getString(cursor.getColumnIndex(
                        ConversationColumns.PARTICIPANT_CONTACT_ID));
            }
        } finally {
            if (cursor != null) {
    ...
    // Hàm update âm báo tin nhắn khi thay đổi cài đặt bên trong tin nhắn
    private void updateRingtone(String id){
        if (id != null) {
            ...
            String[] projectionMessage = new String[]{
                    ConversationColumns.PARTICIPANT_CONTACT_ID,
                    DatabaseHelper.ConversationColumns.NOTIFICATION_SOUND_URI
            };
            try {
                cursorMessage = getContext().getContentResolver().query(MessagingContentProvider.CONVERSATIONS_URI_ALL, projectionMessage, ConversationColumns.PARTICIPANT_CONTACT_ID + " = ?", args, null);
                if (cursorMessage != null && cursorMessage.getCount() > 0) {
                    cursorMessage.moveToFirst();
                    String uriRingtone = cursorMessage.getString(cursorMessage.getColumnIndex(DatabaseHelper.ConversationColumns.NOTIFICATION_SOUND_URI));
                    values.put(ContactLoader.CUSTOM_RINGTONE_SMS, uriRingtone);
                }
            } finally {
            ...
            getContext().getContentResolver().update(ContactsContract.Contacts.CONTENT_URI, values, ContactsContract.Contacts.NAME_RAW_CONTACT_ID + " = ?", args);
        }
    }
    // Bkav HaiKH - Fix bug BOS-3732- End
