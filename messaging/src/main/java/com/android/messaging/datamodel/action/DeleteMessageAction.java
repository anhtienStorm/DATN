...
public class DeleteMessageAction extends Action implements Parcelable {
    ...
    protected Bundle doBackgroundWork() {
        ...
        if (!TextUtils.isEmpty(messageId)) {
            ...
            if (message != null) {
                // Bkav QuangNDb neu la tin nhan audio thi check xoa file audio trong may di
                for (MessagePartData part : message.getParts()) {
                    if (part.isAudio()) {
                        Cursor cursor = Factory.get().getApplicationContext().getContentResolver().query(part.getContentUri(),
