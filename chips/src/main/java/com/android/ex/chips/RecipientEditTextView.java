...
public class RecipientEditTextView extends MultiAutoCompleteTextView implements
    ...
    public RecipientEditTextView(Context context, AttributeSet attrs) {
        ...
        // Bkav HuyNQN comment de bo viec chan selection long click vao text trong toolbar
//        setCustomSelectionActionModeCallback(this);
    ...
    /*package*/ public DrawableRecipientChip getLastChip() {
    ...
    //Bkav QuangNDb xu ly selection change
    @Override
    ...
    public void onSizeChanged(int width, int height, int oldw, int oldh) {
        ...
        if (mScrollView == null && !mTriedGettingScrollView) {
            ...
            if (parent != null) {
                ...
                // Bkav QuangNDb them ham hide scrollbar de override lai
                hideScrollBar();
            }
    ...
    public void clearSelectedChip() {
        ...
        // Bkav HuyNQN comment 2 dong duoi lai de co the selection chip
        /*setCursorVisible(true);
        setSelection(getText().length());*/
    }
     ...
     */
    // Visible for testing.
    /* package */protected void removeChip(DrawableRecipientChip chip) {
        Spannable spannable = getSpannable();
    ...
    /*package*/ void replaceChip(DrawableRecipientChip chip, RecipientEntry entry) {
        ...
        appendRecipientEntryListener(entry);
        int start = getChipStart(chip);
        int end = getChipEnd(chip);
        getSpannable().removeSpan(chip);
        Editable editable = getText();
        ...
        if (chipText != null) {
            if (start == -1 || end == -1) {
                ...
                // TODO: 19/12/2017 QuangNDb doan nay gg code bi thua, tren message mac dinh cung bi
                /** do da insert o ham @link appendRecipientEntry()*/
//                editable.insert(0, chipText);
        ...
        for (int i = 0; i < clip.getItemCount(); i++) {
            ...
            if (!TextUtils.isEmpty(pastedItem)) {
                ...
                if (start < 0 || end < 1) {
                    ...
                    // Bkav HuyNQN sua lai tu append thanh replace de ghi de
                    editable.replace(start, end, pastedItem);
                }
    ...
    /* package */ArrayList<DrawableRecipientChip> handlePaste() {
        ...
        if (isCompletedToken(lastAddress)) {
            ...
            if(tokenStart != -1){ // TODO TrungTH tam thoi fix loi, check lai sau sao lai phai check them so cuoi trong khi da add roi
    ...
    public void appendRecipientEntry(final RecipientEntry entry) {
        ...
        if (chip != null) {
            if (editable.toString().contains(chip.toString())) {//Bkav QuangNDb check neu da ton tai thi thoi khong insert nua
                return;
            }else {
    ...
    public void removeRecipientEntry(final RecipientEntry entry) {
        ...
        for (final DrawableRecipientChip recipient : recips) {
            ...
            if (existingEntry != null && existingEntry.isValid() &&
                ...
                removeRecipientEntryListener(entry);
            }
        }
    }
    ...
    /**
     * Bkav QuangNDb them ham hide scrollbar
     */
    protected void hideScrollBar() {
        // Bkav QuangNDb khong lam gi
    }

    /**Bkav QuangNDb thong bao cho client ve viec xoa lien he gui tin nhan*/
    protected void removeRecipientEntryListener(RecipientEntry entry) {
    }

    /**Bkav QuangNDb thong bao cho client ve viec them lien he*/
